...
¿Qué significa para que un valor tener *identidad*? Si tienes (o emprender) la dirección de memoria de un valor y usar de forma segura, a continuación, el valor tiene identidad. De este modo, que puedes hacer algo más que comparar el contenido de los valores: puedes comparar o distinguirlas por su identidad.

Un *valor l* tiene identidad. Ahora es una cuestión de interés solo histórica que "l" en "valor"l es una abreviatura de "izquierda" (como en la izquierda lado de una asignación). En C++, un valor l puede aparecer en la izquierda *o* la derecha de una asignación. "L" en "valores l", a continuación, no realmente te ayuda a comprender ni definir cuáles son. Solo necesitas comprender que lo llamamos a un valor l es un valor que tiene identidad.

Ejemplos de expresiones que son valores l: una variable con nombre o constante; o una función que devuelve una referencia. Ejemplos de expresiones que son valores *no* l: un temporal; o una función que devuelva por valor.
...
Un valor de l, por el contrario, no móvil, tal como se muestra en la siguiente ilustración. Un valor de l que mueve sería Enfréntese a la definición de *valor l*y sería un problema inesperado código que muy razonablemente para que puedan continuar teniendo acceso al valor de l.

![Un valor r es móvil; no es un valor l](images/is-movable.png)
...
A continuación, hay referencias const valor l (`T const&`), lo que hacen referencia a objetos en el que el usuario de la referencia *no* se escriben (por ejemplo, una constante).
...
La sintaxis para una referencia a un valor r de tipo `T` se escribe como `T&&`. Una referencia de valor r hace referencia a un valor móvil&mdash;un valor cuyo contenido no necesitamos conservar una vez que hemos usado lo (por ejemplo, un temporal). Dado que todo el punto de transición de (lo que modificar) el valor depende de una referencia de valor r, `const` y `volatile` calificadores (también conocida como VC-calificadores) no se aplican a las referencias de valor r.
...
Una referencia de valor r se enlaza a un valor r. De hecho, en términos de resolución de sobrecarga, un valor r *prefiere* se puede enlazar a una referencia de valor r a una referencia const valor l. Pero una referencia de valor r no se puede enlazar a un valor l porque, como hemos dicho, una referencia de valor r hace referencia a un valor cuyo contenido se supone que no necesitamos conservar (por ejemplo, el parámetro de un constructor de movimiento).
...
## <a name="a-glvalue-has-identity-a-prvalue-does-not"></a>Un glvalue tiene identidad; no es un prvalue
En esta etapa, sabemos lo que tiene identidad. Y sabemos lo que es móvil y qué no es. Pero aún no hemos, denominado el conjunto de valores que *no* tienen identidades. Ese conjunto se conoce como el *prvalue*o *valores de r pura*.
...
![Un valor l tiene identidad; no es un prvalue](images/has-identity2.png)

## <a name="the-complete-picture-of-value-categories"></a>La imagen completa de las categorías de valor
Solo permanece combinar la información e ilustraciones anteriormente en una imagen única y grande.
...
Un valor l (un tipo de glvalue) tiene identidad, pero no móvil. Estos son valores por lo general, lectura y escritura que se pasen alrededor de referencia o referencia const o valor si copiar es barata. Un valor l no se puede enlazar a una referencia de valor r.

### <a name="xvalue-im"></a>xValue (i\ & m)
Un xvalue (un tipo de glvalue, pero también un tipo de valor r) tiene identidad y también es móvil. Esto puede ser un valor l erstwhile que hayas decidido mover porque copiar es caro y se cuidado de no tener acceso a él más adelante. Aquí te mostramos cómo puede convertir un valor l en un xvalue.
...
Un prvalue (valores de r pura; un tipo de valor r) no tiene identidad, pero es móvil. Por lo general, estos son temporales, devuelve el resultado de llamar a una función por valor, o el resultado de la evaluación de cualquier otra expresión que no es un glvalue
...
¿Sin embargo, es una referencia de valor r propio un valor r? Una referencia a valores r *sin nombre* (como los que se muestra en los ejemplos de código xvalue anteriores) es un xvalue por lo tanto, sí, es un valor r. Prefiere se puede enlazar a un parámetro de función de referencia de valor r, como las de un constructor de movimiento. Por el contrario (y quizás counter-intuitively), si una referencia de valor r tiene un nombre, a continuación, la expresión que consta de ese nombre es un valor l. Por lo tanto, *no* se puede enlazar a un parámetro de referencia de valor r. Pero es fácil para que sea hacerlo&mdash;simplemente convertirlo en una referencia a valores de r sin nombre (un xvalue) nuevo.
...
Varias referencias like en una expresión (una referencia de valor l a una referencia de valor l o una referencia de valor r a una referencia de valor r) cancelar una fuera otro.
...
- El motivo por el reenvío de referencias (tales como `_Ty&&`) existe es *no* para optimización, pero para realizar lo que se pasa en los mismos y reenviarlos en forma transparente y eficaz. Es probable que producen una referencia de reenvío solo si escribir (o estudias) código de la biblioteca&mdash;por ejemplo, una función de fábrica que reenvía los argumentos de constructor.
