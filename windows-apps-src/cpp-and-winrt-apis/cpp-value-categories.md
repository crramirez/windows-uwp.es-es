...
¿Qué significa para que un valor tener *identidad*? Si tienes (o emprender) la dirección de memoria de un valor y usar de forma segura, a continuación, el valor tiene identidad. De este modo, puede hacer algo más que comparar el contenido de los valores: pueden comparar o distinguirlas por su identidad.

Un *valor l* tiene identidad. Ahora es una cuestión de interés solo histórica que "l" en "valor"l es una abreviación de "izquierda" (como en la izquierda lado de una asignación). En C++, un valor l puede aparecer en la izquierda *o* la derecha de una asignación. "L" en "valores l", a continuación, no realmente te ayuda a comprender ni definir cuáles son. Sólo necesita comprender que lo llamamos a un valor l es un valor que tiene identidad.
...
Ahora, si bien es una instrucción true que valores l tiene identidades, por lo tanto, hacer xvalues. Vaya más en qué una *xvalue* es más adelante en este tema. Por ahora, simplemente Ten en cuenta que hay una categoría de valor que glvalue, se denomina "generalizadas valor l". El superconjunto de glvalues contiene valores l (también conocida como *valores l clásicos*) y xvalues. Por lo tanto, mientras que "un valor l tiene identidad" es true, el conjunto completo de las cosas que tienen identidad es el conjunto de glvalues, tal como se muestra en la siguiente ilustración.
...
"R" en "valor"r es la abreviatura de "derecha" (como en el derecho lado de una asignación). Pero puedes usar valores r y referencias a valores de r, fuera de las asignaciones. La "r" de valores "r", a continuación, no es lo que se centran en. Sólo necesita comprender que lo llamamos a un valor r es un valor que es móvil.

Un valor de l, por el contrario, no móvil, como se muestra en la siguiente ilustración. Un valor de l movido sería Enfréntese a la definición de *valor l*y sería un problema inesperado código que muy razonablemente para que puedan continuar teniendo acceso al valor de l.
...
A continuación, hay referencias constantes valor l (`T const&`), lo que hacen referencia a objetos en el que el usuario de la referencia *no* se escriben (por ejemplo, una constante).
...
En esta etapa, sabemos lo que tiene la identidad. Y sabemos lo que es móvil y qué no es. Pero aún no hemos, denominado el conjunto de valores que *no* tienen identidad. Ese conjunto se conoce como el *prvalue*o *valores de r pura*.
...
Un valor l (un tipo de glvalue) tiene identidad, pero no móvil. Estos son los valores de normalmente de lectura y escritura que pasas alrededor de referencia o referencia const o valor si copiar es barata. Un valor l no se puede enlazar a una referencia de valor r.

### <a name="xvalue-im"></a>xValue (i\ & m)
Un xvalue (un tipo de glvalue, sino también un tipo de valor r) tiene identidad y también es móvil. Esto puede ser un valor l erstwhile que hayas decidido mover porque copiar es costoso y se cuidado de no tener acceso a él más adelante. Aquí te mostramos cómo puede convertir un valor l en un xvalue.
...
Varias referencias like en una expresión (una referencia de valor l con un valor l de referencia o una referencia de valor r a una referencia de valor r) cancelar una salida de otro.
...
- El motivo por el reenvío de referencias (tales como `_Ty&&`) existe es *no* para la optimización, pero para realizar lo que se pasa en los mismos y reenviar en forma transparente y eficaz. Es probable que producen una referencia de reenvío solo si escribir (o estudias) código de la biblioteca&mdash;por ejemplo, una función de fábrica que reenvía los argumentos de constructor.
