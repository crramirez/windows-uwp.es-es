...
Un xvalue (un tipo de glvalue, sino también un tipo de valor r) tiene identidad y también es móvil. Esto puede ser un valor de l erstwhile que hayas decidido mover porque copiar es caro y se cuidado de no tener acceso a él más adelante. Aquí te mostramos cómo puede convertir un valor l en un xvalue.
...
¿Sin embargo, es una referencia de valor r propio un valor r? Una referencia a valores r *sin nombre* (como los que se muestra en los ejemplos de código de xvalue anteriores) es un xvalue por lo tanto, sí, es un valor r. Prefiere se puede enlazar a un parámetro de función de referencia de valor r, como los de un constructor de movimiento. Por el contrario (y quizás counter-intuitively), si una referencia de valor r tiene un nombre, a continuación, la expresión que consta de ese nombre es un valor l. Por lo tanto, *no* se puede enlazar a un parámetro de referencia de valor r. Pero es fácil para que sea hacerlo&mdash;simplemente convertirlo en una referencia a valores de r sin nombre (un xvalue) nuevo.
...
Varias referencias like en una expresión (una referencia de valor l a una referencia de valor l o una referencia de valor r a una referencia de valor r) cancelar una fuera de otra.
...
- El motivo por el reenvío de referencias (tales como `_Ty&&`) existe es *no* para optimización, pero para realizar lo que se pasa en los mismos y reenviarlos en forma transparente y eficaz. Es probable que producen una referencia de reenvío solo si escribir (o estudias) código de la biblioteca&mdash;por ejemplo, una función de fábrica que reenvía en el argumento de constructor.
