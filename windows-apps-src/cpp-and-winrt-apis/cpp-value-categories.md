...
¿Qué significa para que un valor tener *identidad*? Si tienes (o emprender) la dirección de memoria de un valor y usar de forma segura, a continuación, el valor tiene identidad. De este modo, puede hacer algo más que comparar el contenido de valores: puede comparar o distinguirlas por su identidad.

Un *valor l* tiene identidad. Ahora es una cuestión de interés solo histórica que el "l" en "valor"l es una abreviatura de "izquierda" (como en la izquierda lado de una asignación). En C++, un valor l puede aparecer en la izquierda *o* la derecha de una asignación. El "l" en "valores l", a continuación, no realmente te ayuda a comprender ni definir qué son. Sólo necesita comprender que lo llamamos a un valor l es un valor que tiene la identidad.
...
Una referencia de valor r se enlaza a un valor r. De hecho, en términos de resolución de sobrecarga, un valor r *prefiere* se puede enlazar a una referencia de valor r a una referencia const de valor l. Pero una referencia de valor r no se puede enlazar a un valor l porque, como hemos dicho, una referencia de valor r hace referencia a un valor cuyo contenido se supone que no necesitamos conservar (por ejemplo, el parámetro de un constructor de movimiento).
...
En esta etapa, sabemos lo que tiene la identidad. Y sabemos qué es móvil y qué no es. Pero aún te no, denominado el conjunto de valores que *no* tienen identidad. Ese conjunto se conoce como el *prvalue*o los *valores de r pura*.
...
Un xvalue (un tipo de glvalue, sino también un tipo de valor r) tiene identidad y también es móvil. Esto puede ser un valor de l erstwhile que hayas decidido mover porque copiar es costoso y se cuidado de no tener acceso a él más adelante. Aquí te mostramos cómo puede convertir un valor l en un xvalue.
...
En esta sección final contrasta referencias de valores r, que ya hemos mencionado, con el concepto diferentes de una *referencia de reenvío*.
...
- Hace referencia los valores de r motivo (como `A&&`) existe está para que puedes crear una sobrecarga que está optimizada para el caso de que se pasan temporal (u otros valores de r).
