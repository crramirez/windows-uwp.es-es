...
ms.locfileid: "5167462"
---
# <a name="strong-and-weak-references-in-cwinrt"></a>Referencias fuertes y débiles en C++ / WinRT

El tiempo de ejecución de Windows es un sistema de recuento de referencia; y en dicho sistema es importante para que saber sobre el significado de y la distinción entre, fuerte y débil referencias (y referencias que no están instaladas, como el puntero implícito *este* ). Como se verá en este tema, saber cómo administrar estas referencias correctamente puede significar la diferencia entre un sistema confiable que se ejecuta sin problemas y que bloquea impredecible. Proporcionando funciones auxiliares que disponen de profundidad en la proyección de lenguaje, [C++ / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) cumple medio en el trabajo de creación de sistemas más complejos simplemente y correctamente.
...
Después del punto donde se destruir la instancia de clase, parece que no directamente llamamos a él otra vez. Pero supuesto objeto asincrónico tiene un puntero *this* a ella e intenta usarlos para copiar el valor almacenado dentro de la instancia de clase. La corrutina es una función miembro y espera que pueda usar su puntero *this* con impunidad.

Con este cambio en el código, nos encontramos con un problema en el paso 4, porque se ha destruido la instancia de clase y *este* ya no es válido. Tan pronto como el objeto asincrónico intenta obtener acceso a la variable dentro de la instancia de clase, se bloquee (o hacer algo totalmente undefined).
...
Destinatario del evento se destruye, pero el controlador de eventos de la expresión lambda dentro de él todavía se suscribe al **evento** . Cuando se genera el evento, la expresión lambda intenta desreferenciar el puntero *this* , que no es válido en ese momento. Por lo tanto, una infracción de acceso de código en el controlador (o en la continuación de una corrutina) intentando usarlo.

> [!IMPORTANT]
> Si se producen una situación como esta, a continuación, tendrás que pensar en el ciclo de vida de *este* objeto; y si el objeto capturado *esta* sobrevive a la captura. Si no es así, captúralo con una referencia fuerte o débil, como a continuación se muestra a continuación.
>
...
La expresión lambda captura automáticamente las variables locales por referencia. Por lo tanto, para este ejemplo, hemos podríamos equivalente escrita esto.
...
Si una referencia fuerte no es adecuada, puede llamar en su lugar [**Implements:: get_weak**](/uwp/cpp-ref-for-winrt/implements#implementsgetweak-function) para recuperar una referencia débil a *este*. Confirma que todavía puede recuperar una referencia fuerte de ella antes de acceder a los miembros.
...
Esta es la forma estándar, convencional para hacer referencia a un objeto y su función miembro. Para hacer esto seguros, puedes&mdash;a partir de la versión 10.0.17763.0 (Windows 10, versión 1809) del Windows SDK&mdash;establecer una referencia fuerte o débil en el punto donde se registra el controlador. En ese momento, el objeto de destinatario del evento se sabe que es aún activo.
