...
Agrega una referencia y busca `\BankAccountWRC\Debug\BankAccountWRC\BankAccountWRC.winmd` (o agrega una referencia de proyecto a proyecto, si los dos proyectos están en la misma solución). Haz clic en **Agregar** y después en **Aceptar**. Ahora compila BankAccountCoreApp. En el improbable caso aparece un error que el archivo de carga `readme.txt` no existe, excluya este archivo desde el proyecto de componente de Windows Runtime, generarlo y después volver a compilar BankAccountCoreApp.
...
Los parámetros de tipo para esos tipos de dos delegado tienen to Promote la ABI, por lo que los parámetros de tipo deben ser tipos de Windows Runtime, demasiado. Que incluye clases en tiempo de ejecución de terceros y primero, así como los tipos primitivos, como cadenas y números. El compilador te ayuda a con un error "*debe ser de tipo WinRT*" Si olvida esa restricción.
...
En primer lugar, el siguiente ejemplo muestra a un delegado firma que no toma cualquier parámetros (esencialmente una señal simple) y, a continuación, que toma una cadena.
...
Ten en cuenta cómo puede agregar al evento tantas delegados de suscripción que deseas. Sin embargo, hay algunos sobrecarga asociada a un evento. Si todo lo que necesitas es una devolución de llamada simple con un solo delegado suscripción, puedes usar [**winrt:: Delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) en su propio.
