...
Cada vez que hagas clic en la ventana, restarás 1 del saldo de la cuenta bancaria. Para demostrar que se genera el evento según lo esperado, coloca un punto de interrupción en la expresión lambda que controla el evento **AccountIsInDebit** , ejecuta la aplicación y haga clic dentro de la ventana.
...
Los parámetros de tipo para esos tipos de dos delegado tienen to Promote la ABI, por lo que los parámetros de tipo deben ser tipos de Windows Runtime, demasiado. Que incluye las clases en tiempo de ejecución de primera - y de terceros, así como los tipos primitivos, como cadenas y números. El compilador te ayuda a con un error de "*debe ser de tipo WinRT*" Si olvida esa restricción.

Si no tienes que pasar los parámetros o los argumentos con tu evento, a continuación, puedes definir su propio tipo de delegado simple de Windows Runtime. El siguiente ejemplo muestra una versión más sencilla de la clase en tiempo de ejecución de **BankAccount** . Declara un tipo de delegado denominado **SignalDelegate** y, a continuación, usa para generar un evento de tipo de señal en lugar de un evento con un parámetro.
...
Si el evento se usa solo internamente dentro de su C++ / WinRT del proyecto (no a través de archivos binarios), a continuación, seguir usan la plantilla de estructura [**winrt::event**](/uwp/cpp-ref-for-winrt/event) , pero se parametrizar con C++ / que no son de Windows Runtime [**winrt:: Delegate WinRT&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) plantilla de estructura, que es un delegado eficaz, recuento de referencia. Admite cualquier número de parámetros y que no están limitados a los tipos de Windows Runtime.
...
Ten en cuenta cómo puede agregar al evento tantas delegados de suscripción que deseas. Sin embargo, hay algunos sobrecarga asociada a un evento. Si todo lo que necesitas es una devolución de llamada simple con solo un delegado de suscripción único, puedes usar [**winrt:: Delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) en su propio.
...
Te recomendamos que pasas eventos y no delegados, como parámetros de función. La función **Agregar** de [**winrt::event**](/uwp/cpp-ref-for-winrt/event) es la única excepción, porque debe pasar a un delegado en ese caso. El motivo para esta directriz es porque delegados pueden tener diferentes formas en diferentes idiomas de Windows Runtime (en términos de si admiten el registro de un cliente o múltiple). Eventos, con su modelo de suscriptor múltiple, constituyen una opción mucho más predecible y coherente.
...
Ten en cuenta que estas directrices no necesariamente se aplican si estás diseñando una API interna. Aunque API internas suelen ser públicas con el tiempo.
