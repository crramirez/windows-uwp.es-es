...
Ahora crea un nuevo proyecto (en tu solución `BankAccountWRC` o en una nueva). Crear un **Visual C++** > **Windows Universal** > **Core App (C++ / WinRT)** del proyecto y asígnale *BankAccountCoreApp*.
...
En primer lugar, el siguiente ejemplo muestra a un delegado firma que no acepta ningún parámetro (esencialmente una señal de sencilla) y, a continuación, que toma una cadena.
...
Te recomendamos que pasas eventos y no delegados, como parámetros de función. La función **Agregar** del [**winrt::event**](/uwp/cpp-ref-for-winrt/event) es la única excepción, ya que se debe pasar a un delegado en ese caso. El motivo para esta directriz es porque los delegados pueden tener diferentes formas en diferentes idiomas de Windows Runtime (en términos de si admiten el registro de un cliente o varias). Eventos, con su modelo de suscriptor múltiple, constituyen una opción mucho más coherente y predecible.
