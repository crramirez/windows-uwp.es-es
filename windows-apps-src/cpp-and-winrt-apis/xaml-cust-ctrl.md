...
> [!NOTE]
> Si quieres que un DP con un tipo de punto flotante, a continuación, realice `double` (`Double` en [MIDL 3.0](/uwp/midl-3/)). Declarar e implementar un DP de tipo `float` (`Single` en MIDL), y, a continuación, establecer un valor para esa DP en marcado XAML, da como resultado el error *no pudo crear un 'Windows.Foundation.Single' desde el texto '<NUMBER>'*.
...
Ahora compila y ejecuta el proyecto. Verás que se enlace la plantilla de control predeterminada para el pincel de fondo y para la etiqueta de la instancia de **BgLabelControl** en el marcado.

En este tutorial se ha mostrado un ejemplo sencillo de un control personalizado (con plantilla) en C++ / WinRT. Puedes realizar tus propios controles personalizados arbitrariamente enriquecida y completas. Por ejemplo, un control personalizado puede adoptar la forma de algo tan complicado como una cuadrícula de datos editable, un Reproductor de vídeo o un visualizador de geometría 3D.
...
Funciones de *Overridable* presenten diferente en proyecciones de lenguaje diferentes. En C#, por ejemplo, funciones reemplazables normalmente aparecen como funciones virtuales protegidas. En C++ / WinRT, que estén protegidos ni virtual, pero podrás seguir anularlos y proporcionar tu propia implementación, como se muestra anteriormente.
