...
-   Puede usar el modo de un solo uso para enlazar un [ **imagen** ](https://msdn.microsoft.com/library/windows/apps/BR242752) a foto del usuario actual.
-   Puede usar el modo unidireccional para enlazar un [ **ListView** ](https://msdn.microsoft.com/library/windows/apps/BR242878) a una colección de artículos de noticias en tiempo real agrupados por sección periódico.
-   Puede usar el modo bidireccional para enlazar un [ **TextBox** ](https://msdn.microsoft.com/library/windows/apps/BR209683) al nombre de un cliente en un formulario.

Independientemente del modo, hay dos tipos de enlace, y como están ambos normalmente declaradas en el marcado de la interfaz de usuario. Puedes usar la [extensión de marcado {x:Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783) o [extensión de marcado {Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782). Incluso puedes usar una combinación de ambos en la misma aplicación, aun en el mismo elemento de la interfaz de usuario. {x: Bind} es una novedad para Windows 10 y tiene un mejor rendimiento. Todos los detalles que se describen en este tema se aplican a ambos tipos de enlace, a menos que especifiquemos explícitamente lo contrario.
...
Si usas C++ / c++ / extensiones de componentes de WinRT o Visual C++ (C++ / c++ / CX), a continuación, deberá agregar el [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) atributo a cualquier clase en tiempo de ejecución que desea usar el [{Binding} ](https://msdn.microsoft.com/library/windows/apps/Mt204782) extensión de marcado con.

> [!IMPORTANT]
> Si usas [C++ / c++ / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), el [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) atributo está disponible si ha instalado el SDK de Windows versión 10.0.17763.0 (Windows 10, versión 1809 ), o una versión posterior. Sin ese atributo, debe implementar la [ICustomPropertyProvider](/uwp/api/windows.ui.xaml.data.icustompropertyprovider) y [ICustomProperty](/uwp/api/windows.ui.xaml.data.icustomproperty) interfaces para poder usar el [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) marcado extensión.

### <a name="binding-object-declared-using-xbind"></a>Objeto de enlace que se declaran usando {x:Bind}

Hay un solo paso que necesitamos ejecutar antes de crear nuestro marcado [{x:Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783). Tenemos que exponer nuestra clase de origen de enlace desde la clase que representa nuestra página de marcado. Para ello, agregue una propiedad (de tipo **HostViewModel** en este caso) a nuestro **MainPage** page (clase).
...
> [!NOTE]
> Cambia a [ **TextBox.Text** ](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.textbox.text) se envían a un enlace bidireccional de origen cuando el [ **TextBox** ](https://msdn.microsoft.com/library/windows/apps/BR209683) pierde el foco y no después de cada pulsación de tecla de usuario.
...
Si usas C++ / c++ / extensiones de componentes de WinRT o Visual C++ (C++ / c++ / CX) a continuación, utilizar el [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) extensión de marcado, deberá agregar el [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) atributo para cualquier clase en tiempo de ejecución que desea enlazar. Para usar [{x: Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783), no necesita ese atributo.
...
El convertidor también tiene los parámetros opcionales: [**ConverterLanguage**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.data.binding.converterlanguage), que permite especificar el idioma que se usará en la conversión, y [ **ConverterParameter**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.data.binding.converterparameter), lo que permite pasar un parámetro el lógica de conversión. Para obtener un ejemplo en el que se use un parámetro de convertidor, consulta [**IValueConverter**](https://msdn.microsoft.com/library/windows/apps/BR209903).

> [!NOTE]
> Si se produce un error en la conversión, no producen una excepción. En lugar de eso, devuelve [**DependencyProperty.UnsetValue**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.dependencyproperty.unsetvalue), que detendrá la transferencia de datos.
...
[{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) no tiene información de tipo de origen de enlace. Cuando ejecutas la aplicación con el depurador adjunto, los errores de enlace aparecen en la ventana **Resultado** en Visual Studio.

## <a name="creating-bindings-in-code"></a>Crear enlaces en el código

**Tenga en cuenta**  en esta sección solo se aplica a [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782), ya que no se puede crear [{x: Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783) enlaces en código. Sin embargo, algunas de las ventajas de {x:Bind} pueden conseguirse con [**DependencyObject.RegisterPropertyChangedCallback**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.dependencyobject.registerpropertychangedcallback.aspx), que te permite registrar notificaciones de cambio en cualquier propiedad de dependencia.
