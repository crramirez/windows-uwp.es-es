...
description: "En estas notas del producto se describen las técnicas para migrar las aplicaciones .NET y Win32 de recursos Win32 heredados a recursos MRT cuando se empaquetan como paquetes AppX."
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, Windows 10, uwp, UWP, mrt, MRT, pri resources, recursos PRI, games, juegos, centennial, Centennial, desktop app converter, convertidor de aplicaciones de escritorio, mui, MUI, satellite assembly, ensamblado satélite"
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>Migración de recursos heredados a la plataforma de administración de recursos de Windows 10

## <a name="executive-summary"></a>Resumen ejecutivo

Con frecuencia, las aplicaciones Win32 están localizadas en diferentes idiomas, lo que amplía el mercado al que se dirigen<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>. Existen muchas formas para localizar las aplicaciones Win32 tradicionales, pero Windows 8 introdujo un [nuevo sistema de administración de recursos](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx) (llamado "MRT" en este documento <a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>) que funciona entre lenguajes de programación y tipos de aplicaciones, además de proporcionar funcionalidad más allá de una simple localización. 

En combinación con la implementación basada en AppX (p. ej., de la Tienda Windows), MRT puede entregar automáticamente los recursos que mejor se aplican a un usuario o dispositivo determinado, lo que minimiza los tamaños de descarga e instalación de la aplicación. Esta reducción de tamaño puede ser significativa para las aplicaciones que tienen una gran cantidad de contenido localizado, quizás del orden de varios *gigabytes* para juegos AAA. Entre las ventajas adicionales de MRT se incluyen listas localizadas en el shell de Windows y la Tienda Windows, una lógica de reserva automática cuando el idioma preferido de un usuario no coincide con los recursos disponibles.
...
<td>El trabajo mínimo indispensable necesario para que el contenido localizado aparezca en el shell de Windows y en la Tienda Windows</td>
<td>Bajo</td>
</tr>
<tr>
...
Este es un ejemplo sencillo de una aplicación que tiene etiquetas de texto en dos botones (`openButton` y `saveButton`), así como un archivo PNG que se usa para un logotipo (`logoImage`). Las etiquetas de texto están localizadas en inglés y alemán, y el logotipo está optimizado para pantallas de escritorio normales (factor de escala del 100%) y teléfonos de alta resolución (factor de escala de 300%) <a name="footnote4_ref" href="#footnote4"><sup>4</sup></a>. Ten en cuenta que en este diagrama se presenta una visión conceptual de nivel superior del modelo; no se corresponde exactamente con la implementación:
...
Después de completar las fases 0 a 3 anteriores, tendrás un "conjunto" de aplicaciones que puede enviarse a la Tienda Windows y que minimizará el tamaño de la descarga e instalación para los usuarios, omitiendo los recursos que no necesiten (por ejemplo, idiomas que no hablen). Se pueden realizar otras mejoras en el tamaño y la funcionalidad de la aplicación siguiendo un último paso. 
...
0. En el archivo XML, agrega el contenido siguiente, donde el <span style="background-color: yellow">texto resaltado</span> se reemplaza por el texto adecuado para la aplicación, en el idioma predeterminado<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>
...
Si editas el XML directamente, abre el archivo `AppxManifest.xml` y realiza los siguientes cambios en los <span style="background-color: lightgreen">valores resaltados</span>; usa el texto *exacto*, no un texto específico para la aplicación<a name="footnote6_ref" href="#footnote6"><sup>6</sup></a>. Estos nombres deben coincidir con el elemento `Names` que creaste en el archivo `.resw`, con el prefijo del esquema `ms-resource:` y el espacio de nombres `Resources/`. 
...
0. Escribe el comando siguiente, sustituyendo "contoso_demo.xml" por un nombre adecuado para tu proyecto y "en-US" por el idioma predeterminado de la aplicación (o mantén en-US si corresponde). Ten en cuenta el archivo xml se crea en el directorio principal (**no** en el directorio del proyecto) ya que no forma parte de la aplicación<a name="footnote7_ref" href="#footnote7"><sup>7</sup></a>:
...
Como contraejemplo, si la aplicación usa una estructura de archivo plana con un solo directorio `Resources` que contiene los archivos `english_strings.dll` y `french_strings.dll`, **no** se asignará bien a MRT. Una estructura mejor sería un directorio `Resources` con subdirectorios y los archivos `en\strings.dll` y `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a>. Ten en cuenta que es posible usar MRT y las ventajas de empaquetado de AppX, incluso si no se puede seguir esta convención de nombres de archivos; solo requiere más trabajo. Unas notas del producto futuras explicarán este caso. 
...
El tiempo de ejecución de .NET generará el evento `AssemblyResolve` siempre que no pueda encontrar los archivos DLL de recursos y, en este punto, el controlador de eventos proporcionado localizará el archivo deseado a través de MRT y devolverá el ensamblado<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>.
...
## <a name="footnotes"></a>Pies de página

Haz clic en el número para volver a la ubicación de origen en el documento.

<a name="footnote1" href="#footnote1_ref">1.</a> En este documento no se proporciona motivación para la localización en sí; se supone que el lector ya es consciente de esto.

<a name="footnote2" href="#footnote2_ref">2.</a> Históricamente esto significaba "Modern Resource Technology", pero se ha quitado el término "Modern". También se puede llamar al administrador de recursos MRM (Modern Resource Manager, administrador de recursos modernos) o PRI (Package Resource Index, índice de recursos del paquete).

<a name="footnote3" href="#footnote3_ref">3.</a> En esta tabla no se incluyen tareas que no son de localización, como proporcionar iconos de aplicación de alta resolución o de contraste alto. Para más información sobre cómo proporcionar varios activos para iconos, consulta MSDN.

<a name="footnote4" href="#footnote4_ref">4.</a> No se tratan aquí los detalles específicos del factor de escala.

<a name="footnote5" href="#footnote5_ref">5.</a> Ten en cuenta que hay restricciones relativas a la longitud de algunas de estas cadenas; consulta [el tema **VisualElements** en MSDN](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx) para más información. 

<a name="footnote6" href="#footnote6_ref">6.</a> No es requisito usar estos nombres exactos para los recursos, puedes elegir los tuyos propios, pero los que elijas deben coincidir exactamente con lo que aparece en el archivo `.resw`.

<a name="footnote7" href="#footnote7_ref">7.</a> Puedes elegir cualquier directorio que quieras, pero asegúrate de sustituirlo en los comandos futuros

<a name="footnote8" href="#footnote8_ref">8.</a> También es posible usar el mismo nombre de archivo base, pero con calificadores incrustados, como `strings.lang-en.dll` y `strings.lang-fr.dll`, pero el uso de directorios con los códigos de idioma es conceptualmente más sencillo, por lo que es en lo que nos centraremos.

<a name="footnote9" href="#footnote9_ref">9.</a> Ten en cuenta que si la aplicación ya tiene un controlador `AssemblyResolve` para otros fines, tendrás que integrar el código de resolución de recursos con el código existente.
