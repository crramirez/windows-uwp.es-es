...
**Nota** Para [propiedades adjuntas](../xaml-platform/attached-properties-overview.md), necesitas una sintaxis especial en la columna Nombre de un archivo .resw. Por ejemplo, para establecer un valor para la propiedad adjunta [AutomationProperties.Name](/uwp/api/windows.ui.xaml.automation.automationproperties?branch=live#Windows_UI_Xaml_Automation_AutomationProperties_NameProperty) del identificador "Greeting", esto es lo que debe especificar en la columna Nombre.
...
1. Abre el manifiesto de paquete de la aplicación (el archivo `Package.appxmanifest`), en el que de manera predeterminada, el nombre para mostrar de la aplicación se expresa como un literal de cadena.
