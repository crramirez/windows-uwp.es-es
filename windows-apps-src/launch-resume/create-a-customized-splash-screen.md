...
ms.locfileid: "66366283"
---
# <a name="display-a-splash-screen-for-more-time"></a>Mostrar una pantalla de presentación durante más tiempo

**API importantes**

-   [SplashScreen (clase)](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen)
...
> [!NOTE]
> La frase "extendido pantalla de presentación" en este tema hace referencia a una pantalla de presentación que permanece en la pantalla durante un largo período de tiempo. No significa una subclase derivada de la [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) clase.
...
-   El código debe usar el [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) coordina la clase para colocar la imagen de pantalla de bienvenida de la aplicación en la misma pantalla como la pantalla de presentación predeterminada.
-   El código debe responder a eventos de cambio de tamaño de ventana (por ejemplo, cuando se gira la pantalla o la aplicación se mueve junto a otra aplicación en la pantalla) mediante el uso de la [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) clase para colocar los elementos de la pantalla de presentación extendido.
...
-   Cambiar el [en segundo plano](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control.backgroundproperty) propiedad del valor predeterminado [cuadrícula](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Grid) elemento para que coincida con el color de fondo establecidos para la pantalla de presentación de la aplicación en el manifiesto de aplicación (en el **activos visuales**sección del archivo Package.appxmanifest). El color de pantalla de presentación predeterminada es un gris claro (valor hexadecimal \#464646). Ten en cuenta que este elemento **Grid** se proporciona de forma predeterminada cuando se crea una nueva **Página en blanco**. No tienes que usar **Grid**; tan solo es una base útil para crear una pantalla de presentación extendida.
-   Agregar un [lienzo](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Canvas) elemento a la [cuadrícula](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Grid). Usarás **Canvas** para posicionar la imagen de la pantalla de presentación extendida.
-   Agregar un [imagen](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Image) elemento a la [lienzo](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Canvas). Usa la misma imagen de 600 x 320 píxeles para la pantalla de presentación extendida que usaste para la pantalla de presentación predeterminada.
-   (Opcional) Agrega un control de progreso para mostrar a los usuarios que tu aplicación se está cargando. En este tema se agrega un [ProgressRing](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ProgressRing), en lugar de una determinada o indeterminado [ProgressBar](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ProgressBar).

En el ejemplo siguiente se muestra un [cuadrícula](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Grid) con estos cambios y adiciones.
...
> [!NOTE]
> Este ejemplo establece el ancho de la [ProgressRing](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ProgressRing) a 20 píxeles. Puedes establecer manualmente el ancho en un valor adecuado para tu aplicación; sin embargo, los controles con un ancho inferior a 20 píxeles no se representarán.
...
1.  **Agregar espacios de nombres necesarios**
    ...
    Deberá agregar los siguientes espacios de nombres para **ExtendedSplash.xaml.cs** para tener acceso a la [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) (clase), el [Rect](https://docs.microsoft.com/uwp/api/windows.foundation.rect) struct y el [ Window.SizeChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.window.sizechanged) eventos.
...
2.  **Crear una clase parcial y declarar variables de clase**
    ...
    Varios métodos usan estas variables de clase. La variable `splashImageRect` almacena las coordenadas en que el sistema mostró la imagen de la pantalla de presentación de la aplicación. El `splash` variable almacena un [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) objeto y el `dismissed` variable realiza un seguimiento de si se ha descartado la pantalla de presentación que se muestra por el sistema.
...
3.  **Definir un constructor para la clase que se coloca correctamente la imagen**
    ...
    Asegúrese de registrar su [Window.SizeChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.window.sizechanged) controlador (`ExtendedSplash_OnResize` en el ejemplo) en el constructor de clase para que la aplicación coloca la imagen correctamente en la pantalla de presentación extendido.
...
5.  **(Opcional) Defina un método de clase para colocar un control de progreso en la pantalla de presentación extendida**
    ...
    Si decide agregar un [ProgressRing](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ProgressRing) a la pantalla de presentación extendida, colóquelo en relación con la imagen de pantalla de bienvenida. Agrega el siguiente código a ExtendedSplash.xaml.cs para centrar **ProgressRing** 32 píxeles por debajo de la imagen.
...
6.  **Dentro de la clase, defina un controlador para el evento descartadas**
    ...
    En ExtendedSplash.xaml.cs, responder cuando el [SplashScreen.Dismissed](https://docs.microsoft.com/uwp/api/windows.applicationmodel.activation.splashscreen.dismissed) se produce el evento estableciendo el `dismissed` variable de clase en true. Si tu aplicación tiene operaciones de configuración, agrégalas a este controlador de eventos.
...
7.  **Dentro de la clase, defina un controlador de eventos Window.SizeChanged**
    ...
    Prepara la pantalla de presentación extendida para volver a posicionar sus elementos si un usuario cambia el tamaño de la ventana. Este código responde cuando un [Window.SizeChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.window.sizechanged) se produce un evento mediante la captura las nuevas coordenadas y cambiar la posición de la imagen. Si agregaste un control de progreso a la pantalla de presentación extendida, vuelve a posicionarlo dentro del controlador de eventos también.
    ...
    > [!NOTE]
    > Antes de intentar obtener la ubicación de la imagen Asegúrese de que la variable de clase (`splash`) contiene válido [SplashScreen](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.SplashScreen) de objeto, como se muestra en el ejemplo.

     

8.  **(Opcional) Agregar un método de clase para restaurar un estado de sesión guardada**

    El código que agregó a la [OnLaunched](https://docs.microsoft.com/uwp/api/windows.ui.xaml.application.onlaunched) método en el paso 4: [Modifique el controlador de activación de lanzamiento](#modify-the-launch-activation-handler) hace que la aplicación mostrar una pantalla de presentación extendida cuando inicia. Para consolidar todos los métodos relacionados con el inicio de la aplicación en la clase de pantalla de presentación extendida, podría considerar la posibilidad de agregar un método al archivo ExtendedSplash.xaml.cs para restaurar el estado de la aplicación.
...
-   Esta aplicación también usa una plantilla de proyecto de aplicación para UWP usa [marco](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Frame) navegación. Como resultado, en App.xaml.cs, el controlador de activación de lanzamiento ([OnLaunched](https://docs.microsoft.com/uwp/api/windows.ui.xaml.application.onlaunched)) define un `rootFrame` y lo usa para establecer el contenido de la ventana de la aplicación.
...
Si no usa una plantilla de proyecto para la aplicación, consulte el paso 4: [Modifique el controlador de activación de lanzamiento](#modify-the-launch-activation-handler) para obtener un ejemplo de una modificada `OnLaunched` que no usa [marco](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Frame) navegación.
...
* [Propiedad Windows.ApplicationModel.Activation.SplashScreen.ImageLocation](https://docs.microsoft.com/uwp/api/windows.applicationmodel.activation.splashscreen.imagelocation)
