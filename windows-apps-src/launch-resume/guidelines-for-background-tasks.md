...
|Desencadenadores disponibles | Las tareas en segundo plano dentro de proceso no admiten los siguientes desencadenadores: [DeviceUseTrigger](/uwp/api/windows.applicationmodel.background.deviceusetrigger?f=255&MSPPError=-2147217396), [DeviceServicingTrigger](/uwp/api/windows.applicationmodel.background.deviceservicingtrigger) e **IoTStartupTask**. |
...
Vea la [Referencia del esquema](/uwp/schemas/appxpackage/uapmanifestschema/element-application) de la aplicación para obtener más información sobre la configuración del grupo de recursos.
...
-   En el método [Run](/uwp/api/windows.applicationmodel.background.ibackgroundtask.run), solicita aplazamientos para todas las llamadas a métodos asincrónicos y ciérralos cuando el método haya terminado. O bien, use un aplazamiento con **Async/Await**.
...
-   No especifique un elemento ejecutable en el manifiesto a menos que esté usando un desencadenador que se debe ejecutar en el mismo contexto que la aplicación (por ejemplo, la [**ControlChannelTrigger**](/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger)).
