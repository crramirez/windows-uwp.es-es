...
* **No se crea actividades para las acciones que los usuarios no necesitan reanudar.** Si la aplicación se usa para completar operaciones simples única que no se conserva el estado, probablemente no es necesario crear una actividad de usuario.
...
* **Aplicaciones basadas en el documento** : crear una actividad por documento, con uno o más elementos del historial que refleja períodos de uso. Es importante para la actividad de actualización que se realizan cambios en el documento.
...
* Los metadatos de contenido son los datos JSON que pueden usarse para agrupar y recuperar las actividades en un contexto específico. Por lo general, esto adopta la forma de http://schema.org datos. Consulta a continuación para obtener instrucciones sobre cómo rellenar un vistazo a estos datos.
...
* Usa información de texto complementario con moderación. 
  * Evitar que la tarjeta se llene con texto y solo usa información complementaria que ayuda a los usuarios en la búsqueda de la actividad derecha o refleja la información de estado (por ejemplo, el progreso actual en una tarea particular).
