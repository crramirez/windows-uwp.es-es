...
Si el usuario estableció directivas de privacidad o de energía que impiden que la aplicación realiza la actividad en segundo plano que puede devolver [DisabledByPolicy](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.applicationtriggerresult) .
Además, ejecutar un único AppTrigger a la vez. Si intentas ejecutar una AppTrigger mientras otro ya se está ejecutando, la función devolverá [CurrentlyRunning](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.applicationtriggerresult).
