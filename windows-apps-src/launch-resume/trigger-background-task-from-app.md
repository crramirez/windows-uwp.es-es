...
En este ejemplo que la condición se establece en **InternetAvailable** que, al desencadenarse, la tarea solo se ejecuta una vez que el acceso a internet está disponible. Para obtener una lista de posibles condiciones, consulta [**SystemConditionType**](https://msdn.microsoft.com/library/windows/apps/br224835).
...
Ten en cuenta que no se puede llamar **ApplicationTrigger.RequestAsync** desde la propia tarea en segundo plano, o cuando la aplicación está en segundo plano ejecuta estado (consulta el [ciclo de vida de aplicación](app-lifecycle.md) para obtener más información acerca de los Estados de la aplicación).
