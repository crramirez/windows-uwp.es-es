...
Volver a la definición de interfaz y, a continuación, pegue el nuevo GUID justo antes de la interfaz de IToaster, tal como se muestra en el siguiente ejemplo. (No uses el GUID en el ejemplo. Todas las interfaces única deben tener su propio GUID).
...
Ahora convertir manualmente el GUID const en un GuidAttribute de modo que está formateada tal como se muestra en el siguiente ejemplo. Ten en cuenta que las llaves se reemplazan con corchetes y paréntesis y se quita el punto y coma final.
```cpp
...
Para agregar un evento posterior a la compilación personalizado, en el Explorador de soluciones, abre el menú contextual para el proyecto de ToasterComponent y, a continuación, elige propiedades. En el panel izquierdo de las páginas de propiedades, seleccione eventos de compilación y, a continuación, elige el botón Editar posterior a la compilación. Agrega los siguientes comandos en la línea de comandos posterior a la compilación. (El archivo por lotes debe llamarse en primer lugar para establecer las variables de entorno para encontrar la herramienta winmdidl.)
...
Para asegurarte de que el archivo IDL se vuelve a generar cada vez que se modifica el archivo .winmd, cambiar **ejecutar el evento posterior a la compilación** para **cuando la compilación actualiza el resultado del proyecto.**
...
Comprueba que la solución compilaciones correctamente seleccionando **compilación > volver a generar solución** en la barra de menús de Visual Studio.
