...
>  En este momento, no se admiten enumeraciones de nota en el contrato de componentes de Windows Runtime entre el componente de escritorio y la aplicación de carga.
...
>**Tenga en cuenta** que la instalación en paralelo se puede activar&gt; en configuración-actualizar&gt; & seguridad-para desarrolladores.
...
Afortunadamente, son fáciles de declarar mediante clases de\# C. Sin embargo, hay consideraciones de rendimiento importantes cuando se definen estas conversaciones (se trata en una sección posterior).
...
La categoría es inProcessServer porque hay varias entradas en la categoría outOfProcessServer que no son aplicables a esta configuración de la aplicación. Tenga en cuenta <Path> que el componente debe contener siempre clrhost. dll (sin embargo, esto no se aplica y especificar un valor diferente producirá un error de manera **no** definida).
...
Para instalar la aplicación, copie el archivo **winmd** de implementación en el directorio correcto especificado en el manifiesto de la aplicación de carga <ActivatableClassAttribute>lateral asociado: valor = "ruta de acceso". Copia también los archivos auxiliares asociados y el archivo DLL de proxy/código auxiliar (este punto se trata con más detalle más adelante). Si no se puede copiar el archivo **winmd** de implementación en la ubicación del directorio del servidor, todas las llamadas de la aplicación que se cargan en paralelo a las nuevas en el RuntimeClass producirán un error "clase no registrada". Si no se instala o no se registra el proxy o código auxiliar, todas las llamadas producirán un error sin valores de retorno. Este último error **no** suele estar asociado a excepciones visibles.
...
>**Nota** Es habitual esperar otras posibles operaciones de ejecución larga mientras se escribe la implementación. En ese caso, debe declararse el código de la **tarea. Run** :
...
c) un \* \_archivo i. c (por ejemplo,\_MyWinRTComponent i. c)

d) un \* \_archivo p. c (por ejemplo,\_MyWinRTComponent p. c)
...
32 \_WINDOWS; REGISTRAR\_DLL\_DE PROXY "

**C/C++ > encabezado precompilado: Seleccione "no se está usando el encabezado precompilado"**
...
-   Al devolver una  *<T>lista*donde ** T es un objeto de una operación asincrónica o de una recopilación de propiedades, se producirá una gran cantidad de conversaciones entre procesos. Por ejemplo, supongamos que devuelve una*lista&lt;&gt;* de objetos People.  Cada pase de iteración será una llamada entre procesos. Cada objeto *People* devuelto se representa mediante un proxy y cada llamada a un método o propiedad en ese objeto individual producirá una llamada entre procesos. Por lo tanto, un objeto de *lista&lt;de personas&gt;*  "inocente" en el que el *recuento* es grande producirá un gran número de llamadas lentas. La transferencia masiva de estructuras del contenido de una matriz ofrece mejor rendimiento. Por ejemplo:
