---
title: Windows Runtime componentes con C# y Visual Basic
...
ms.locfileid: "70393727"
---
# <a name="windows-runtime-components-with-c-and-visual-basic"></a>Windows Runtime componentes con C# y Visual Basic

Puede usar código administrado para crear sus propios tipos de Windows Runtime y empaquetarlos en un componente Windows Runtime. Puede usar el componente en aplicaciones Plataforma universal de Windows (UWP) escritas en C++, JavaScript, Visual Basic o. C# En este tema se describen las reglas para crear un componente y se explican algunos aspectos de la compatibilidad de .NET con el Windows Runtime. En general, esa compatibilidad está diseñada para ser transparente para el programador de .NET. Sin embargo, cuando creas un componente para su uso con JavaScript o C++, debes tener en cuenta las diferencias en la forma en que esos lenguajes son compatibles con Windows Runtime.
...
Para implementar un método asincrónico en el componente, agregue "Async" al final del nombre del método y devuelva una de las interfaces Windows Runtime que representen acciones o operaciones asincrónicas: **IAsyncAction**, **IAsyncActionWithProgress&lt;TProgress&gt;** , **IAsyncOperation&lt;TResulto&gt;** **IAsyncOperationWithProgressTResult,&lt;TProgress &gt;** .
...
Un método que usa `await` (`Await` en Visual Basic) requiere la `async` palabra clave`Async` (en Visual Basic). Si expone un método de este tipo desde un componente de Windows Runtime, `async` aplique la palabra clave al delegado que pasa al método **Run** .
