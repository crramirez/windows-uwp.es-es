...
En Windows 10, puedes enviar un paquete de aplicación, un lote de aplicaciones o un archivo de carga del paquete de aplicación completa al [Centro de partners](https://partner.microsoft.com/dashboard). De estas opciones, enviar un archivo de carga del paquete de aplicación proporcionará la mejor experiencia.
...
Si distribuyes la aplicación a través de Microsoft Store, Visual Studio puede asociar el paquete con la tienda. Para ello, haz clic en el nombre del proyecto en el Explorador de soluciones y elige **almacén**->**Asociar aplicación con la tienda**. También puede hacer esto en el Asistente para **Crear paquetes de aplicaciones** , que se describe en la siguiente sección. Cuando asocias tu aplicación, se actualizan automáticamente algunos de los campos en la pestaña Empaquetado del diseñador de manifiestos.

## <a name="create-an-app-package-upload-file"></a>Crear un archivo de carga del paquete de aplicación

Para distribuir una aplicación a través de Microsoft Store debe crear un paquete de aplicación (.appx o .msix), un lote de aplicaciones (.appxbundle o .msixbundle), o un archivo de carga del paquete de aplicación (.appxupload o .msixupload) y [enviar la aplicación empaquetada al centro de partners](https://docs.microsoft.com/windows/uwp/publish/app-submissions). Aunque es posible enviar un lote de aplicación o paquete de aplicaciones al centro de partners por sí mismo, te recomendamos que podrás enviar un archivo de carga del paquete de aplicación. Puedes crear un archivo de carga del paquete de aplicación mediante el uso del Asistente para **Crear paquetes de aplicaciones** en Visual Studio o puedes crear manualmente a partir de los paquetes de aplicación existentes o los lotes de aplicaciones.
...
### <a name="to-create-your-app-package-upload-file-using-visual-studio"></a>Para crear el archivo de carga del paquete de aplicación con Visual Studio
...
3.  Selecciona **quiero crear paquetes para cargarlos en la Microsoft Store con un nuevo nombre de la aplicación** en el primer cuadro de diálogo y, a continuación, haz clic en **siguiente**.  
    ![Visualización de la ventana de diálogo Crear los paquetes](images/packaging-screen3.jpg)

    Si tu proyecto ya se ha asociado con una aplicación en la tienda, también tiene una opción para crear paquetes para la aplicación de Store asociado. Si decides **quiero crear paquetes de instalación de prueba**, Visual Studio no generará el archivo paquete de aplicación carga (.msixupload o .appxupload) para envíos del centro de partners. Si solo quieres realizar la instalación de prueba de la aplicación para ejecutarla en dispositivos internos o para pruebas, puedes seleccionar esta opción. Para obtener más información acerca de la instalación de prueba, consulta [Habilitar el dispositivo para el desarrollo](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development).
...
5.  Selecciona el nombre de la aplicación para el paquete de la lista de aplicaciones actualmente registrada en tu cuenta o reserva uno nuevo si todavía no lo has hecho en el centro de partners.  
6.  Asegúrate de seleccionar las tres configuraciones de arquitectura (x86, x64 y ARM) en el diálogo **Seleccionar y configurar paquetes** para garantizar que tu aplicación se puede implementar en la gama de dispositivos más amplia. En el cuadro de lista **Crear lote de aplicaciones**, selecciona **Siempre**. Un lote de aplicaciones (.appxbundle o .msixbundle) se prefiere un archivo de paquete de aplicación única porque contiene una colección de paquetes de aplicación configurados para cada tipo de arquitectura de procesador. Cuando eliges generar la recopilación de aplicación, la recopilación de aplicación se incluirá en el archivo paquete final carga (.appxupload o .msixupload) junto con la información de depuración a análisis de bloqueo. Si no estás seguro de qué arquitecturas elegir o quieres obtener más información sobre qué arquitecturas se usan en varios dispositivos, consulta [Arquitecturas de paquete de aplicación](https://docs.microsoft.com/windows/uwp/packaging/device-architecture).  
...
9.  Haz clic en **Crear** para generar el paquete de aplicación. Si ha seleccionado una de las opciones **quiero crear paquetes para cargarlos en la Microsoft Store** en el paso 3 y estás creando un paquete para el envío al centro de partners, el asistente creará un archivo de carga (.appxupload o .msixupload) del paquete. Si has seleccionado **quiero crear paquetes de instalación de prueba** en el paso 3, el asistente creará un paquete de aplicación única o un lote de aplicaciones en función de tus selecciones en el paso 6.
10. Cuando la aplicación se haya empaquetado correctamente, verás este cuadro de diálogo y se puede recuperar el archivo de carga del paquete de aplicación desde la ubicación de salida especificado. En este punto, puedes [validar el paquete de aplicación en el equipo local o un equipo remoto](#validate-your-app-package).
...
1. Coloca los siguientes archivos en una carpeta:
    ...
    - Un archivo .appxsym. Se trata de un archivo .pdb comprimido que contiene los símbolos públicos de la aplicación que se usa para [análisis de bloqueo](../publish/health-report.md) en el centro de partners. Puedes omitir este archivo, pero si lo haces, ninguna información de análisis o de depuración de bloqueos estará disponible para la aplicación.
2. Comprime la carpeta.
3. Cambiar el nombre de extensión de la carpeta comprimida de .zip a .msixupload o .appxupload.

### <a name="validate-your-app-package"></a>Validar el paquete de aplicación

Valida la aplicación antes de enviarla al centro de partners para la certificación en un equipo local o remoto. Solo se pueden validar las compilaciones de versiones del paquete de la aplicación, no las compilaciones de depuración. Para obtener más información sobre el envío de la aplicación Centro de partners, consulta [envíos de aplicaciones](https://docs.microsoft.com/windows/uwp/publish/app-submissions).

**Para validar localmente el paquete de la aplicación**

1. En la página de **Creación del paquete completada** final del Asistente para **Crear paquetes de aplicaciones** , deje seleccionada la opción de **equipo Local** y haz clic en el **Kit de certificación de aplicaciones de inicio de Windows**. Para obtener más información sobre la prueba de la aplicación con el Kit para la certificación de hardware en Windows, consulta [Kit para la certificación de aplicaciones en Windows](https://msdn.microsoft.com/library/windows/apps/Mt186449).
...
2. Una vez WACK haya terminado y la aplicación haya pasado la certificación, estás listo para enviar la aplicación Centro de partners. Asegúrate de cargar el archivo correcto. La ubicación predeterminada del archivo puede encontrarse en la carpeta raíz de la solución `\[AppName]\AppPackages` y termina con la extensión de archivo .appxupload o .msixupload. El nombre tendrá el formato `[AppName]_[AppVersion]_x86_x64_arm_bundle.appxupload` o `[AppName]_[AppVersion]_x86_x64_arm_bundle.msixupload` si has optado por una recopilación de aplicación con toda la arquitectura del paquete seleccionada.

**Para validar el paquete de la aplicación en un dispositivo remoto de Windows 10**
...
Con los paquetes de aplicación para UWP, las aplicaciones no se instalan en un dispositivo ya que están con aplicaciones de escritorio. Por lo general, descargas aplicaciones para UWP desde Microsoft Store, que también instala la aplicación en el dispositivo para ti. Las aplicaciones se pueden instalar sin publicarlas en la Store (instalación de prueba). Esto te permite instalar y aplicaciones de prueba con el paquete de la aplicación de archivo que haya creado. Si tienes una aplicación que no quieres vender en la Store, como una aplicación de línea de negocio (LOB), puedes realizar la instalación de prueba de dicha aplicación para que otros usuarios de la empresa pueden usarla.

Antes de poder transferir localmente la aplicación en un dispositivo de destino, debes [Habilitar el dispositivo para el desarrollo](../get-started/enable-your-device-for-development.md).

Para transferir localmente la aplicación en un dispositivo Windows 10 Mobile, usa la herramienta [WinAppDeployCmd.exe](install-universal-windows-apps-with-the-winappdeploycmd-tool.md) . Para equipos de escritorio, portátiles y tabletas, siguen las instrucciones que aparecen a continuación.
...
3.  Haz clic con el botón derecho en el archivo **Add-AppDevPackage.ps1**. Elige **Ejecutar con PowerShell** y sigue las instrucciones.  
    ...
    >[!TIP]
    > Para abrir el menú contextual en una tableta, toca la pantalla donde quieras con el botón derecho, sostén hasta que aparezca un círculo completo y después levanta el dedo. El menú contextual se abre después de levantar el dedo.
