...
|---------------|--------|---------------|------|
| applicationId | string | El [identificador de la Store](in-app-purchases-and-trials.md#store-ids) de la aplicación para la que quieres recuperar los datos de embudo de adquisiciones. Un ejemplo de un id. de la Tienda sería 9WZDNCRFJ3Q8. |  Sí  |
| startDate | fecha | La fecha de inicio del intervalo de fechas de los datos de embudo de adquisiciones que se recuperarán. El valor predeterminado es la fecha actual. |  No  |
| endDate | fecha | La fecha de finalización del intervalo de fechas de los datos de embudo de adquisiciones que se recuperarán. El valor predeterminado es la fecha actual. |  No  |
...
| TotalCount | entero    | El número total de objetos en la matriz *Value*.        |
