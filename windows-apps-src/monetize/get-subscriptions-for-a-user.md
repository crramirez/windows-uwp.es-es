...
|----------------|--------|-----------------|----------|
| b2bKey         | string | La [clave de identificador de Microsoft Store](view-and-grant-products-from-a-service.md#step-4) que represente la identidad del usuario cuyas suscripciones quieres obtener.  | Sí      |
| continuationToken |  string     |  Si el usuario tiene derecho a varias suscripciones, el cuerpo de la respuesta devuelve un token de continuación cuando se alcanza el límite de la página. Proporciona ese token de continuación aquí en llamadas posteriores para recuperar los productos restantes.    | No      |
| pageSize       | string | Número máximo de suscripciones que puede devolver una respuesta. El valor predeterminado es 25.     |  No      |
...
|---------------|--------|---------------------|
| items | array | Una matriz de objetos que contienen datos sobre cada complemento de una suscripción que el usuario especificado tiene derecho a usar. Para más información sobre los datos de cada objeto, consulta la tabla siguiente.  |
