...
El espacio de nombres de seguridad le permite bloquear el dispositivo, compruebe la lista de procesos del sistema y de usuario, obtener las direcciones IP y MAC y desactive los recursos almacenados en caché web.
...
Bloquea el dispositivo. También se usa para desbloquear el dispositivo. La aplicación web de pruebas invocará a esta llamada antes de permitir a los estudiantes que inicien las pruebas. El implementador es necesario para realizar todas las acciones necesarias para proteger el entorno de pruebas. Los pasos tomados para proteger el entorno de dispositivo específico y, por ejemplo, incluyen aspectos como capturas de pantalla Deshabilitar Deshabilitar chat de voz en el modo seguro, borrar el Portapapeles del sistema, escriba en un modo de quiosco, deshabilitando espacios en OSX 10.7 + los dispositivos, etcetera. La aplicación de prueba le permitirá bloqueo antes de una evaluación comienza y deshabilitará el bloqueo cuando el estudiante ha completado la evaluación y está fuera de la prueba seguro.
...
Obtén información acerca de la plataforma en el que se ejecuta la aplicación de prueba. Esto se utiliza para ampliar cualquier información que sea apreciable desde el agente de usuario.
...
* `callback` -La función que se va a llamar cuando se complete esta función. Debe tener la forma `Function(String infoObj)` donde `infoObj` es una cadena JSON que contiene varios campos. Se deben admitir los siguientes campos:
    ...
    * `model` representa el modelo de dispositivo para dispositivos móviles solo; es null o no se usa para los exploradores de escritorio.
...
**Parámetros**  
* `restart` : Este parámetro se omite pero debe proporcionarse.
...
**Parámetros**  
* `callback` -La función que se invoca cuando se complete esta llamada. Debe tener el formato: `Function(Boolean permissiveMode)` donde `permissiveMode` indica si el explorador se encuentra actualmente en el modo permisivo. Si no está definido o es nulo, se produjo un error en la operación get.
...
**Parámetros**  
* `enable` -El valor booleano que indica el estado del modo previsto de permisiva.  
* `callback` -La función que se invoca cuando se complete esta llamada. Debe tener el formato: `Function(Boolean permissiveMode)` donde `permissiveMode` indica si el explorador se encuentra actualmente en el modo permisivo. Si no está definido o es nulo, se produjo un error en la operación set.
...
**Parámetros**  
* `callback` -La función que se invoca cuando se complete esta llamada. Debe tener el formato: `Function(String addressArray)` donde `addressArray` tiene el formato: `"['00:11:22:33:44:55','etc']"`.
...
**Parámetros**  
`feature` -La cadena para determinar qué capacidad de consulta. Las cadenas de funcionalidad válidas son "screenMonitoring", "printing" y "textSuggestions" (no distingue mayúsculas de minúsculas).
...
**Parámetros**  
* `feature` -La cadena para determinar cuál funcionalidad debe establecer. Las cadenas de funcionalidad válidas son `"screenMonitoring"`, `"printing"` y `"textSuggestions"` (no distingue mayúsculas de minúsculas).  
* `value` -La configuración deseada para la característica. Debe ser `"true"` o `"false"`.  
* `onSuccess` -[opcional] la función que llama una vez que se ha completado correctamente la operación de establecimiento. Debe tener el formato `Function(String jsonValue)` donde *jsonValue* tiene el formato: `{<feature>:true|false|undefined}`.  
* `onError` -[opcional] la función que se invoca si el error en la operación de conjunto. Debe tener el formato `Function(String jsonValue)` donde *jsonValue* tiene el formato: `{<feature>:true|false|undefined}`.
