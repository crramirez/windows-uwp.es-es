...
A continuación, tendrás dos carpetas, [antes](https://github.com/Microsoft/Windows-tutorials-web/tree/master/BabylonJS-game-with-WebVR/before) y [después](https://github.com/Microsoft/Windows-tutorials-web/tree/master/BabylonJS-game-with-WebVR/after). La carpeta "antes" es nuestro juego antes de agregaron las características VR y la carpeta "después" es el juego terminado con soporte técnico VR.
...
-   **js /** : una carpeta que contiene los archivos de JavaScript. El archivo main.js es nuestro juego, y los demás archivos son las bibliotecas que se usan.
-   **modelos /** : una carpeta que contiene los modelos 3D. Este juego tenemos un solo modelo para el dinosaurio.
-   **index.html** : la página Web que hospeda al representador del juego. Abrir esta página en Microsoft Edge, inicia el juego.

Puedes probar las dos versiones del juego abriendo sus archivos respectivos index.html en Microsoft Edge.
...
Si se cumplen todos los requisitos, a continuación, puede activar las funciones para desarrolladores y simular unos auriculares de realidad mixta de Windows conectado al equipo. Si estás suerte de tener un auriculares real cercanos, conectarlo y ejecuta el programa de instalación.
...
[Babylon.GUI](https://doc.babylonjs.com/how_to/gui) es una biblioteca de cuentan con asistencia de VR, lo que te permite para crear simple, interfaces de usuario interactivas que funcionan bien para VR y no VR muestra.
...
Con Babylon.js, se puede agregar WebVR rápidamente mediante el uso de la [`WebVRFreeCamera`](http://doc.babylonjs.com/classes/3.1/webvrfreecamera). Esta cámara puede tomar la entrada de teclado y te permite usar auriculares VR para controlar la rotación de "head".
...
Un clic en el juego ahora crea un mensaje similar al siguiente o muestra el juego en los auriculares al instante si el usuario ha aceptado la solicitud antes.
...
Dado que la `WebVRFreeCamera` inicialmente no es compatible con los controladores para juegos, te enviaremos nuestros botones del controlador para juegos se asignan a las teclas de dirección del teclado. Haremos lo esto por seguir leyendo el `inputs` propiedad de la cámara. Al agregar los códigos correspondientes para el stick analógico izquierdo hacia arriba, abajo, izquierda y derecha para que coincida con las teclas de dirección, nuestro controlador para juegos es nuevo en acción.
