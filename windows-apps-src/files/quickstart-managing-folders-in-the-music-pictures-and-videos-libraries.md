...
> [!NOTE]
> Recuerde que debe declarar la funcionalidad apropiada. Para más información, consulte [Declaraciones de funcionalidad de las aplicaciones](../packaging/app-capability-declarations.md).
 

Para obtener una referencia a la biblioteca de música, imágenes o vídeos del usuario, llama al método [**StorageLibrary.GetLibraryAsync**](/uwp/api/windows.storage.storagelibrary.getlibraryasync). Proporciona el valor correspondiente de la enumeración [**KnownLibraryId**](/uwp/api/Windows.Storage.KnownLibraryId).
...
Para obtener la lista de carpetas de una biblioteca, obtén el valor de la propiedad [**StorageLibrary.Folders**](/uwp/api/windows.storage.storagelibrary.folders).
...
Para obtener la carpeta de una biblioteca donde se guardan los nuevos archivos de forma predeterminada, obtén el valor de la propiedad [**StorageLibrary.SaveFolder**](/uwp/api/windows.storage.storagelibrary.savefolder).
...
Para agregar una carpeta a una biblioteca, llama al método [**StorageLibrary.RequestAddFolderAsync**](/uwp/api/windows.storage.storagelibrary.requestaddfolderasync). Si tomamos como ejemplo la biblioteca de imágenes, la llamada a este método hace que se muestre un selector de carpetas al usuario con un botón **Agregar esta carpeta a Imágenes**. Si el usuario elige una carpeta, la carpeta permanece en su ubicación original en el disco y se convierte en un elemento de la propiedad [**StorageLibrary.Folders**](/uwp/api/windows.storage.storagelibrary.folders) (y en la aplicación Fotos integrada), aunque la carpeta no aparece como elemento secundario de la carpeta Imágenes en el Explorador de archivos.
...
Para quitar una carpeta de una biblioteca, llama al método [**StorageLibrary.RequestRemoveFolderAsync**](/uwp/api/windows.storage.storagelibrary.requestremovefolderasync) y especifica la carpeta que quieres quitar. Puedes usar [**StorageLibrary.Folders**](/uwp/api/windows.storage.storagelibrary.folders) y un control [**ListView**](/uwp/api/Windows.UI.Xaml.Controls.ListView) (o similar) para que el usuario seleccione una carpeta para quitar.

Cuando se llama a [**StorageLibrary.RequestRemoveFolderAsync**](/uwp/api/windows.storage.storagelibrary.requestremovefolderasync), el usuario verá un cuadro de diálogo de confirmación que indica que la carpeta "ya no aparecerá en Imágenes, pero no se eliminará". Esto significa que la carpeta permanece en su ubicación original en el disco, se quitará de la propiedad [**StorageLibrary.Folders**](/uwp/api/windows.storage.storagelibrary.folders) y ya no se incluirá en la aplicación Fotos integrada.

El siguiente ejemplo se da por hecho que el usuario ha seleccionado la carpeta que quiere quitar en un control [**ListView**](/uwp/api/Windows.UI.Xaml.Controls.ListView) denominado **lvPictureFolders**.
...
Para obtener notificaciones sobre los cambios en la lista de carpetas de una biblioteca, registra un controlador para el evento [**StorageLibrary.DefinitionChanged**](/uwp/api/windows.storage.storagelibrary.definitionchanged) de la biblioteca.
...
No tienes que escribir código adicional para administrar esta posibilidad. Los métodos del espacio de nombres de [**Windows.Storage**](/uwp/api/Windows.Storage) que consultan las carpetas conocidas combinan con transparencia los resultados de la consulta de ambas ubicaciones. Para obtener estos resultados combinados, no necesitas especificar la función **removableStorage** en el archivo de manifiesto de la aplicación.
...
Si quieres que el usuario pueda volver a abrir una foto en la aplicación que la capturó, puedes guardar el **CreatorAppId** con los metadatos de la foto; usa para ello un código similar al del ejemplo siguiente. En este ejemplo, **testPhoto** es un objeto [**StorageFile**](/uwp/api/Windows.Storage.StorageFile).
