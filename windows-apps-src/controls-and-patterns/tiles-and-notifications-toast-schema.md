...
Si prefieres utilizar XML sin formato en lugar de la [Biblioteca Notificaciones](https://www.nuget.org/packages/Microsoft.Toolkit.Uwp.Notifications/), consulta [el esquema XML](tiles-and-notifications-toast-xml-schema.md).
...
| Propiedad | Tipo | Obligatorio |Descripción |
|---|---|---|---|
| **Text** | cadena o BindableString | false | El texto que se va a mostrar. Se ha agregado la compatibilidad de enlace de datos en la actualización Creators Update, pero solo funciona para los elementos de texto de nivel superior. |
...
| Propiedad | Tipo | Obligatorio | Descripción |
|---|---|---|---|
| **url** | uri | false | El archivo multimedia que se reproducirá en lugar del sonido predeterminado. Solo se admiten ms-appx y ms-appdata. |
| **loop** | boolean | false | Establece en true si el sonido se debe repetir mientras se muestra la notificación del sistema; false para reproducirlo solo una sola vez (predeterminado). |
| **silent** | boolean | false | True para silenciar el sonido; false para permitir que se reproduzca el sonido de notificación del sistema (predeterminado). |


## <a name="toastheader"></a>ToastHeader
Encabezado personalizado que agrupa varias notificaciones en el centro de actividades.
...
| **title** | cadena | true | Título para el encabezado. |
| **action**| Acción | false | Declara qué acción se realizará cuando el usuario haga clic en la notificación del sistema. |
