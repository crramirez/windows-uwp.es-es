...
title: Optimizar las animaciones, los recursos multimedia y las imágenes
...
-   Ejecutar cualquier animación que se considere independiente en un elemento con la propiedad [**CacheMode**](https://msdn.microsoft.com/library/windows/apps/BR208911-cachemode) establecida en **BitmapCache**. Esta acción se considera dependiente porque debe volver a rasterizarse la memoria caché de cada fotograma.

### No animes los controles WebView ni MediaElement

El marco XAML no representa directamente el contenido web de un control [**WebView**](https://msdn.microsoft.com/library/windows/apps/BR227702), el cual necesita que se componga trabajo adicional con el resto de la escena. Este trabajo extra se acumula al animar el control alrededor de la pantalla y puede crear problemas de sincronización (por ejemplo, es posible que el contenido HTML no se mueva en sincronía con el resto del contenido XAML de la página). Cuando necesites animar un control **WebView**, intercámbialo por un control [**WebViewBrush**](https://msdn.microsoft.com/library/windows/apps/BR227702brush) mientras dure la animación.

También es mala idea animar una clase [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926). Además del daño que produzca al rendimiento, puede causar la desactivación u otras anomalías en las imágenes del vídeo que se reproduce.

### Usar las aplicaciones infinitas moderadamente

La mayoría de las animaciones se ejecutan durante un tiempo especificado, pero si estableces la propiedad [**Timeline.Duration**](https://msdn.microsoft.com/library/windows/apps/BR243207) en "Forever", permitirás que la animación se ejecute indefinidamente. Te recomendamos que reduzcas al mínimo el uso de las animaciones infinitas, ya que consumen recursos de la CPU de forma constante y pueden impedir que esta entre en estado inactivo o de baja energía y, de este modo, la energía se agota con mayor rapidez.

La adición de un controlador de [**CompositionTarget.Rendering**](https://msdn.microsoft.com/library/windows/apps/BR228127) es similar a realizar la ejecución de una animación infinita. Normalmente, el subproceso de interfaz de usuario solo está activo cuando tiene trabajo que realizar, pero al agregar un controlador para este evento se fuerza su ejecución en cada fotograma. Quita el controlador cuando no haya trabajo por realizar y regístralo cuando se vuelva a necesitar.

### Usar la biblioteca de animaciones
...
Puedes animar cada propiedad de clase [**CompositeTransform3D**](https://msdn.microsoft.com/library/windows/apps/Dn914714) de forma independiente; por lo tanto, aplica solo las animaciones que necesites. Para ver ejemplos y obtener más información, consulta [**UIElement.Transform3D**](https://msdn.microsoft.com/library/windows/apps/BR208911-transform3d). Para más información sobre la animación de transformaciones, consulta [Animaciones con guion gráfico](https://msdn.microsoft.com/library/windows/apps/Mt187354) y [Animaciones de fotograma clave y animaciones de función de aceleración](https://msdn.microsoft.com/library/windows/apps/Mt187352).
...
### Mostrar la reproducción de vídeo en pantalla completa siempre que sea posible

En aplicaciones para UWP, usa siempre la propiedad [**IsFullWindow**](https://msdn.microsoft.com/library/windows/apps/BR242926-isfullwindow) en [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) para habilitar y deshabilitar la representación en ventana completa. Esto garantiza que se usen optimizaciones en el nivel del sistema durante la reproducción multimedia.

El marco XAML puede optimizar la presentación del contenido de vídeo cuando es el único objeto que se representa, lo que da como resultado una experiencia que usa menos energía y produce velocidades de fotogramas más altas. Para optimizar la reproducción multimedia, establece el tamaño de un objeto [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) en el ancho y alto de la pantalla, y no muestres otros elementos XAML.

Existen motivos razonables para superponer elementos XAML sobre un objeto [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) que ocupa el ancho y alto total de la pantalla; por ejemplo, puedes superponer subtítulos o controles de transporte momentáneos. Si te aseguras de ocultar estos elementos (por ejemplo, estableciendo Visibility=”Collapsed”) cuando no son necesarios, la reproducción multimedia vuelve a recuperar su estado más eficaz.

### Desactivación de pantalla y ahorro de energía

Para evitar que se desactive la pantalla cuando no se detecte ninguna acción del usuario (por ejemplo, cuando una aplicación reproduce un vídeo), puedes llamar a [**DisplayRequest.RequestActive**](https://msdn.microsoft.com/library/windows/apps/BR241818).

Para ahorrar energía y alargar la duración de la batería, debes llamar a [**DisplayRequest.RequestRelease**](https://msdn.microsoft.com/library/windows/apps/BR241819) para liberar la solicitud de la pantalla cuando ya no sea necesaria.
...
### Colocar los demás elementos al costado del vídeo insertado

A menudo, las aplicaciones ofrecen una vista incrustada donde se reproduce vídeo dentro de una página. En este caso, pierdes claramente la optimización de la pantalla completa porque el objeto [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) no es del tamaño de la página y hay dibujados otros objetos XAML. Ten cuidado de no dibujar un borde alrededor de un objeto **MediaElement**, ya que entrarás en este modo por equivocación.

No dibujes elementos XAML sobre el vídeo cuando se encuentre en modo incrustado. Si lo haces, se fuerza al marco a realizar trabajo adicional para componer la escena. Por ejemplo, para optimizar esta situación, puedes situar los controles de transporte debajo de un elemento multimedia incrustado en lugar de colocarlos sobre el vídeo. En esta imagen, la barra roja indica un conjunto de controles de transporte (Reproducir, Pausar, Detener, etc.).

![MediaElement con elementos superpuestos](images/videowithoverlay.png)
...
![MediaElement con elementos vecinos](images/videowithneighbors.png)

### Retrasar la configuración del origen de un MediaElement

Los motores multimedia son objetos que consumen muchos recursos, y el marco XAML retrasa el mayor tiempo posible la carga de los dll y la creación de objetos de gran tamaño. Debido a ello, se fuerza a la clase [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) a que realice este trabajo una vez establecido su origen mediante la propiedad [**Source**](https://msdn.microsoft.com/library/windows/apps/BR242926-source) o el método [**SetSource**](https://msdn.microsoft.com/library/windows/apps/BR242926-setsource). Al establecerlos cuando el usuario está realmente listo para reproducir el elemento multimedia, se retrasa tanto como es posible la mayor parte del uso de recursos asociado con **MediaElement**.

### Establecer MediaElement.PosterSource

Al establecer [**MediaElement.PosterSource**](https://msdn.microsoft.com/library/windows/apps/BR242926-postersource), se permite al XAML liberar algunos recursos de GPU que, de otro modo, se habrían usado. Esta API permite a una aplicación usar la menor cantidad de memoria posible.

### Mejorar el arrastre del cabezal de reproducción multimedia
...
-   Enlaza el valor de una clase [**Slider**](https://msdn.microsoft.com/library/windows/apps/BR209614) a [**MediaElement.Position**](https://msdn.microsoft.com/library/windows/apps/BR242926-position) o actualízalo en función de un temporizador. No hagas las dos cosas. Si eliges la segunda opción, asegúrate de usar una frecuencia de actualización razonable para el temporizador. El marco XAML solo actualiza **MediaElement.Position** cada 250 milisegundos durante la reproducción.
-   El tamaño de la frecuencia de los pasos de la clase Slider debe escalarse con la longitud del vídeo.
-   Suscríbete a los eventos [**PointerPressed**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerpressed), [**PointerMoved**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointermoved) y [**PointerReleased**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerreleased) del control deslizante para establecer la propiedad [**MediaElement.PlaybackRate**](https://msdn.microsoft.com/library/windows/apps/BR242926-playbackrate) en 0, cuando el usuario arrastre el control de posición del control deslizante.
-   En el controlador de eventos [**PointerReleased**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerreleased), establece de forma manual la posición multimedia en el valor de la posición del control deslizante para ajustar el control de posición óptimo al arrastrar el cabezal de reproducción.

### Igualar la resolución de vídeo con la resolución del dispositivo
...
### Elegir los formatos recomendados
...
Cuando incluyas efectos de audio breves de latencia baja, por ejemplo en juegos, usa archivos WAV con datos PCM sin comprimir para reducir la carga de procesamiento que suele haber en los formatos comprimidos de audio.

### Descarga de audio por hardware

Para que la descarga de audio por hardware se aplique automáticamente, debes establecer la propiedad [**MediaElement.AudioCategory**](https://msdn.microsoft.com/library/windows/apps/BR242926-audiocategory) en **ForegroundOnlyMedia** o **BackgroundCapableMedia**. La descarga de audio por hardware optimiza la representación de audio, lo que puede mejorar la funcionalidad y la duración de la batería.
...
-   La imagen se oculta al establecer la propiedad [**Opacity**](https://msdn.microsoft.com/library/windows/apps/BR208962) en 0 o [**Visibility**](https://msdn.microsoft.com/library/windows/apps/BR208911-visibility) en **Collapsed** en el elemento de imagen host, en el pincel o en cualquier elemento primario.
...
Ejemplo 2; código subyacente (malo): configurar el elemento UriSource de BitmapImage antes de conectarlo al árbol.
...
En consecuencia, deberías usar [**UriSource**](https://msdn.microsoft.com/library/windows/apps/BR243235-urisource) cuando se carguen imágenes desde un paquete de la aplicación, y evitar el uso de una secuencia de archivos y de [**SetSourceAsync**](https://msdn.microsoft.com/library/windows/apps/JJ191522) cuando no sea necesario.
...
### Imágenes de rasterización de software
...
### Carga de imágenes de subproceso en segundo plano
...
La clase [**SoftwareBitmapSource**](https://msdn.microsoft.com/library/windows/apps/Dn997854) intercambia imágenes sin comprimir interoperables entre distintos espacios de nombres de WinRT como [**BitmapDecoder**](https://msdn.microsoft.com/library/windows/apps/BR226176), API de cámara y XAML. Esta clase omite una copia adicional que normalmente sería necesaria con [**WriteableBitmap**](https://msdn.microsoft.com/library/windows/apps/BR243259), y que es útil para reducir la latencia de origen a la pantalla y el uso máximo de la memoria.

También puedes configurar la clase [**SoftwareBitmap**](https://msdn.microsoft.com/library/windows/apps/Dn887358) que suministra información de origen, para usar una interfaz [**IWICBitmap**](https://msdn.microsoft.com/library/windows/desktop/Ee719675) personalizada y así poder proporcionar un almacén de respaldo recargable que permita a la aplicación volver a asignar la memoria como considere apropiado. Este es un caso de uso avanzado de C++.

La aplicación debería usar [**SoftwareBitmap**](https://msdn.microsoft.com/library/windows/apps/Dn887358) y [**SoftwareBitmapSource**](https://msdn.microsoft.com/library/windows/apps/Dn997854) para interoperar con otras API de WinRT que produzcan y consuman imágenes. Asimismo, también debería usar la clase **SoftwareBitmapSource** al cargar los datos de la imagen sin comprimir, en lugar de usar [**WriteableBitmap**](https://msdn.microsoft.com/library/windows/apps/BR243259).

### Usar GetThumbnailAsync para las miniaturas

Un caso de uso de escalado de imágenes es la creación de miniaturas. Si bien puedes usar [**DecodePixelWidth**](https://msdn.microsoft.com/library/windows/apps/BR243243) y [**DecodePixelHeight**](https://msdn.microsoft.com/library/windows/apps/BR243241) para proporcionar versiones pequeñas de las imágenes, la plataforma universal de Windows (UWP) ofrece API aún más eficaces para la recuperación de miniaturas. [**GetThumbnailAsync**](https://msdn.microsoft.com/library/windows/apps/BR227210) proporciona las miniaturas de imágenes que ya tienen almacenado en caché el sistema de archivos. Esto proporciona un rendimiento incluso mejor que el que ofrecen las API de XAML, porque no es necesario abrir o descodificar la imagen.
...
### Descodificar las imágenes una sola vez
