...
description: Obtén información sobre cómo obtener e interpretar los datos de códigos de barras digitalizar.
ms.author: elcowle
ms.date: 08/29/2018
ms.topic: article
ms.prod: windows
...
Obtener el tipo de etiqueta de código de barras descodificada es bastante sencilla&mdash;simplemente llamamos [GetName](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodesymbologies.getname) en **ScanDataType**.
...
Para obtener la etiqueta de código de barras descodificada, hay algunas cosas que tienes que tener en cuenta. Solo determinados tipos de datos contienen texto codificado, por lo que debe comprobar si la simbología puede convertir a una cadena y, a continuación, convertir el búfer que se obtienen de **ScanDataLabel** a una cadena codificada de UTF-8.
...
Para obtener completo, los datos sin procesar desde el código de barras, simplemente convertimos el búfer que se obtienen de **ScanData** en una cadena.
...
Algunos productos pueden marcarse con varias etiquetas, a veces se denomina *multisymbol etiquetas* o *etiquetas en capas*. Estos códigos de barras normalmente se organizan verticalmente y pueden ser de simbología de iguales o diferente. Si el escáner lee los artículos que contienen varias etiquetas, cada código de barras se entrega a la aplicación como una etiqueta independiente. Esto es necesario debido a la actual falta de normalización de estos tipos de códigos de barras. Uno no es capaz de determinar todas las variaciones en función de los datos de códigos de barras individuales. Por lo tanto, la aplicación que determinar cuándo un código de barras de etiqueta varios se ha leído en función de los datos devueltos. (Ten en cuenta que un escáner puede admite o no la lectura de varias etiquetas.)

Este valor se establece antes de un evento **DataReceived** que se genera a la aplicación.
