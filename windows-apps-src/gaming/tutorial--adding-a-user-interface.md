...
ms.locfileid: "4691397"
---
# <a name="add-a-user-interface"></a>Agregar una interfaz de usuario


Ahora que nuestro juego tiene sus vistas en 3D en su lugar, es tiempo para centrarse en agregar algunos elementos 2D para que el juego puede proporcionar comentarios sobre el estado del Reproductor. Esto puede realizarlo agregar sencillas opciones de menú y salida de la canalización de componentes de visualización frontal encima de los gráficos 3D.
...
Con Direct2D, agrega un número de elementos gráficos de interfaz de usuario y comportamientos a nuestro juego de DirectX de UWP incluidos:
...
1. Una pantalla de visualización frontal para la puntuación y los controles en el juego.
...
- Dos intersección segmentos de línea que se utiliza para formar una forma de manera cruzada
...
Dividir el método abajo más allá, esta parte del [**gamehud:: Render**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameHud.cpp#L320-L358) método dibuja nuestro movimiento y disparo rectángulos con [**ID2D1RenderTarget::DrawRectangle**](https://msdn.microsoft.com/library/windows/desktop/dd371902)y cruz con dos llamadas a [**ID2D1RenderTarget::DrawLine**](https://msdn.microsoft.com/library/windows/desktop/dd371895).
...
Además de la pantalla de visualización frontal, la muestra de juego tiene una superposición que representa seis estados del juego. Todos los Estados de características de un primitivo de gran rectángulo negro con texto para el jugador lo lea. Los rectángulos de controlador de movimiento y vista y cruz no se dibuja porque no están activos en estos Estados.
...
La superposición se divide en dos secciones: **estado** y la **acción**. La sección de **estado** más se desglosa en rectángulos de **título** y **cuerpo** . La sección **acción** solo tiene un rectángulo. Cada rectángulo tiene un propósito diferente.

-   `titleRectangle` contiene el texto del título.
-   `bodyRectangle` contiene el texto de cuerpo.
...
Dependiendo de qué está sucediendo en el juego, el **estado** y la sección **acción** se ajustan los campos de texto.
...
- **DrawText** se llama para dibujar el texto que se almacenan en `m_titleString`, `m_bodyString`, y `m_actionString` en el rectángulo de adecuado con el correspondiente **ID2D1SolidColorBrush**.
- Se llama a [**ID2D1RenderTarget::EndDraw**](ID2D1RenderTarget::EndDraw) para detener todas las operaciones de dibujos en `m_levelBitmap`.
- Se crea otro mapa de bits con **CreateBitmap** denominado `m_tooSmallBitmap` usar como reserva, que muestra solo si la configuración de pantalla es demasiado pequeña para el juego.
