...
Si desea saber si se admite una determinada API de .NET en la versión de UWP a la que está destinada, puede consultar la referencia de la [API de .net Standard](/dotnet/api/index?view=netstandard-2.0) y seleccionar la versión de la .net Standard compatible con esa versión de UWP.
...
Es habitual que los juegos serialicen los datos guardados para que los jugadores no puedan manipularlos fácilmente. Sin embargo, [BinaryFormatter](/dotnet/api/system.runtime.serialization.formatters.binary.binaryformatter), que serializa un objeto en binario, no está disponible en versiones anteriores del .net Standard (anterior a 2,0). Considere la posibilidad de usar [XmlSerializer](/dotnet/api/system.xml.serialization.xmlserializer) o [DataContractJsonSerializer](/dotnet/api/system.runtime.serialization.json.datacontractjsonserializer) en su lugar.
...
Algunos tipos del espacio de nombres [System.IO](/dotnet/api/system.io) , como [FileStream](/dotnet/api/system.io.filestream), no están disponibles en versiones anteriores de la .net Standard. Sin embargo, Unity proporciona los tipos de [directorio](/dotnet/api/system.io.directory), [archivo](/dotnet/api/system.io.file)y **FileStream** para poder usarlos en el juego.

Como alternativa, puede usar las API de [Windows. Storage](/uwp/api/Windows.Storage) , que solo están disponibles para las aplicaciones de UWP. Sin embargo, estas API restringen la aplicación a escribir en su almacenamiento específico y no proporcionan acceso gratuito a todo el sistema de archivos. Consulte [archivos, carpetas y bibliotecas](../files/index.md) para obtener más información.

Una nota importante es que el método [Close](/dotnet/api/system.io.stream.close) solo está disponible en .net Standard 2,0 y versiones posteriores (aunque Unity proporciona un método de extensión). Use [Dispose](/dotnet/api/system.io.stream.dispose) en su lugar.

### <a name="threading"></a>Subprocesos

Algunos tipos de los espacios de nombres [System. Threading](/dotnet/api/system.threading) , como [ThreadPool](/dotnet/api/system.threading.threadpool), no están disponibles en versiones anteriores del .net Standard. En estos casos, puede usar el [Windows.SysTEM. ](/uwp/api/windows.system.threading) Espacio de nombres de subprocesos en su lugar.
...
Consulte [seguridad](../security/index.md) para obtener más información sobre el uso de las API de seguridad de WinRT.

### <a name="networking"></a>Redes

Parte de la **red del sistema &period; .** * los espacios de nombres, como [System .net. mail](/dotnet/api/system.net.mail?view=netstandard-2.0), tampoco están disponibles al compilar un juego Unity para UWP. Para la mayoría de estas API, use el correspondiente **Windows. networking.** * y **Windows. Web.** * API de WinRT para obtener una funcionalidad similar. Vea [redes y servicios web](../networking/index.md) para obtener más información.

En el caso de **System .net. mail**, use el espacio de nombres [Windows. ApplicationModel. email](/uwp/api/windows.applicationmodel.email) . Consulte [envío de correo electrónico](../contacts-and-calendar/sending-email.md) para obtener más información.
