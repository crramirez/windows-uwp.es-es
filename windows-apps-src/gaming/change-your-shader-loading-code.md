...
Establecer una ventanilla en Direct3D 11 es muy similar a como lo haces en OpenGL ES 2.0. En Direct3D 11, llame a [**ID3D11DeviceContext:: RSSetViewports**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-rssetviewports) con una [**ventanilla\_** ](https://docs.microsoft.com/previous-versions/windows/desktop/legacy/jj151722(v=vs.85))configurada.
...
| OpenGL ES 2.0 | Direct3D 11                                                                                                                                  |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| glViewport    | [**CD3D11\_ventanilla**](https://docs.microsoft.com/previous-versions/windows/desktop/legacy/jj151722(v=vs.85)), [ **ID3D11DeviceContext:: RSSetViewports**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-rssetviewports) |
...
| GLSL ES para OpenGL ES 2.0 | 1.40. Las implementaciones anteriores de GLSL ES para OpenGL ES 2.0 pueden usar de 1.10 a 1.30. Compruebe el código original con glGetString (GL\_\_idioma\_versión) o glGetString (SOMBREAdo\_idioma\_versión) para determinarlo. | ~SM 2.0               |
...
La semántica que tiene el prefijo "VP\_" es la semántica del valor del sistema que escribe el programa del sombreador; la propia aplicación (que se ejecuta en la CPU) no puede modificarla. Normalmente, contiene los valores de entradas y salidas de otra fase del sombreador en la canalización de gráficos o se generan totalmente por la GPU).

Además, la semántica de\_ SV tiene comportamientos diferentes cuando se usan para especificar la entrada o salida de una fase de sombreador. Por ejemplo, SV\_posición (salida) contiene los datos de vértice transformados durante la etapa del sombreador de vértices y la posición de\_(entrada) contiene los valores de posición en píxeles interpolados durante la rasterización.
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Posición del\_de contabilidad        | POSITION(n) para los datos del búfer de vértices. La posición\_VP proporciona una posición en píxeles al sombreador de píxeles y no se puede escribir en la aplicación.                                        |
...
| GL\_FragData\[n\]   | SV\_destino\[n\] para escribir desde un sombreador de píxeles en una textura de destino u otro búfer de píxeles.                                                                               |
...
En este caso, VP\_destino es la ubicación del destino de representación en la que se escribe el color de píxel (definido como vector con cuatro valores float) cuando el sombreador finaliza la ejecución.
