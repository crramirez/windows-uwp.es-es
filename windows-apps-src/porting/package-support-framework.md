...
Puedes usar la [Corrección de la redirección de archivos](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) para redirigir los intentos de escritura o lectura de datos en un directorio que no sea accesible desde una aplicación que se ejecuta en un contenedor MSIX.
...
Asegúrate de revisar las contribuciones de la Comunidad a nuestra página de [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) . Es posible que otros desarrolladores han resuelto un problema similar a la tuya y han compartido una corrección de tiempo de ejecución.
...
| correcciones | config | (Opcional) Controla cómo se comporta la lista de distribución de corrección. El formato exacto de este valor varía en una base de corrección por corrección como cada corrección puede interpretar este blob de"", como los que quiere. |

El `applications`, `processes`, y `fixups` las claves son matrices. Esto significa que puedes usar el archivo config.json para especificar más de una aplicación, procesos y corrección DLL.
...
Puedes usar Visual Studio para depurar una corrección de tiempo de ejecución, ampliar una corrección de tiempo de ejecución o crear uno desde cero. Tendrás que seguir estos pasos para tener éxito.
...
>[!NOTE]
> Si no tienes el código fuente para la aplicación, elige el proyecto de selector PSF. Te mostraremos cómo se hace referencia el archivo ejecutable cuando se crea un archivo de configuración.
...
| correcciones | config | (Opcional) Controla cómo se comporta la DLL de corrección. El formato exacto de este valor varía en una base de corrección por corrección como cada corrección puede interpretar este blob de"", como los que quiere. |
...
>[!NOTE]
> Si bien Visual Studio ofrece el desarrollo más sencillo y experiencia de depuración, existen algunas limitaciones, por lo tanto, más adelante en esta guía, analizaremos otras técnicas de depuración que se pueden aplicar.
...
Declarar la ``FIXUP_DEFINE_EXPORTS`` macro y, a continuación, agrega una declaración de inclusión de la `fixup_framework.h` en la parte superior de cada uno. Archivo CPP donde vas a agregar las funciones de la corrección de tiempo de ejecución.
