...
Puedes aplicar una corrección de tiempo de ejecución existente con algunas herramientas sencillas del Windows SDK y siguiendo estos pasos.
...
Puedes obtener el paquete de Nuget PSF mediante Visual Studio. También puedes obtener con la herramienta de línea de comandos de Nuget independiente.
...
| applications | ejecutable | La ruta de acceso del paquete relativa al archivo ejecutable que quieres iniciar. En la mayoría de los casos, puedes obtener este valor desde el archivo de manifiesto de paquete antes de modificarlas. Es el valor de la `Executable` atributo de la `Application` elemento. |
...
| correcciones | config | (Opcional) Controla cómo se comporta la lista de distribución de corrección. El formato exacto de este valor varía de forma de corrección mediante la corrección como cada reparación puede interpretar este blob de"", como los que quiere. |
...
Con el botón secundario que del proyecto y, a continuación, elige **las propiedades**.
...
| applications | workingDirectory | (Opcional) Una ruta de acceso relativa de paquete que se usará como el directorio de trabajo de la aplicación que se inicia. Si no estableces este valor, el sistema operativo usa el `System32` directorio como directorio de trabajo de la aplicación. |
| procesos | ejecutable | En la mayoría de los casos, este será el nombre de la `executable` configurado anteriormente con la extensión de archivo y ruta de acceso quitada. |
| correcciones | DLL | Ruta de acceso de paquete relativa a la corrección DLL para cargar. |
| correcciones | config | (Opcional) Controla cómo se comporta la DLL de corrección. El formato exacto de este valor varía de forma de corrección mediante la corrección como cada reparación puede interpretar este blob de"", como los que quiere. |
...
Declarar la ``FIXUP_DEFINE_EXPORTS`` macro y, a continuación, agrega una declaración de incluir para la `fixup_framework.h` en la parte superior de cada uno. Archivo CPP donde se va a agregar las funciones de la corrección de tiempo de ejecución.
...
g
```
(se ejecuta hasta que la aplicación de destino se inicia y se entra en el depurador)
