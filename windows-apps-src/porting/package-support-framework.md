...
## <a name="find-a-runtime-fix"></a>Encontrar una corrección de tiempo de ejecución
...
Puedes usar la [Corrección de redireccionamiento de archivo](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) para redirigir intentos de escritura o lectura de los datos en un directorio que no sea accesible desde una aplicación que se ejecuta en un contenedor MSIX.
...
Asegúrate de revisar las contribuciones de la Comunidad a nuestra página de [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) . Es posible que otros desarrolladores han resuelve un problema similar a la tuya y han compartido una corrección de tiempo de ejecución.
...
Si ya tienes un archivo .msix (o .appx), puede desempaquetar su contenido en una carpeta de diseño que se usará como el área de ensayo para el paquete.  Puedes hacerlo desde una **x64 nativo símbolo de herramientas para VS 2017**, o manualmente con la ruta de acceso de ubicación SDK en la ruta de acceso de búsqueda ejecutable.
...
### <a name="add-the-package-support-framework-files-to-your-package"></a>Agregar los archivos de marco de soporte técnico de paquete al paquete
...
| applications | ejecutable | La ruta de acceso relativa de paquete al archivo ejecutable que quieres iniciar. En la mayoría de los casos, puedes obtener este valor desde el archivo de manifiesto de paquete antes de modificarlas. Es el valor de la `Executable` atributo de la `Application` elemento. |
...
| correcciones | configuración | (Opcional) Controla cómo se comporta la lista de distribución de corrección. El formato exacto de este valor varía según la corrección mediante la corrección como cada corrección puede interpretar este blob de"", como los que quiere. |
...
## <a name="debug-extend-or-create-a-runtime-fix"></a>Depurar, ampliar o crear una corrección de tiempo de ejecución

Puedes usar Visual Studio para depurar una corrección de tiempo de ejecución, ampliar una corrección de tiempo de ejecución o crear uno desde cero. Tendrás que hacer lo se realice correctamente.
...
Buscar el *PSF** Nuget empaquetar y, a continuación, se instala para este proyecto.

![paquete de NuGet](images/desktop-to-uwp/psf-package.png)

Si desea depurar o ampliar una corrección de tiempo de ejecución existente, agregar los archivos de corrección de tiempo de ejecución que obtienen mediante el uso de las instrucciones que se describen en la sección [encontrar una corrección de tiempo de ejecución](#find) de esta guía.

Si vas a crear una solución nueva, no agregues nada en este proyecto aún. Te ayudaremos a agregar los archivos correctos a este proyecto más adelante en esta guía. Por ahora, seguiremos configurar la solución.
...
Agrega un proyecto de **Proyecto vacío** de C++ a la solución.
...
![Referencia PSF launcher](images/desktop-to-uwp/shim-exe-reference.png)
...
>[!NOTE]
> Si no tienes el código fuente para la aplicación, elige el proyecto de iniciador PSF. Te mostraremos cómo se hace referencia el archivo ejecutable cuando se crea un archivo de configuración.
...
Agrega un archivo denominado ``config.json`` a tu proyecto de empaquetado, a continuación, copia y pega el texto de json siguiente en el archivo. Establece la propiedad de la **Acción de paquete** para **el contenido**.
...
| correcciones | configuración | (Opcional) Controla cómo se comporta la DLL de corrección. El formato exacto de este valor varía según la corrección mediante la corrección como cada corrección puede interpretar este blob de"", como los que quiere. |
...
Declarar la ``FIXUP_DEFINE_EXPORTS`` macro y, a continuación, agrega una declaración de incluir para el `fixup_framework.h` en la parte superior de cada uno. Archivo CPP donde vas a agregar las funciones de la corrección de tiempo de ejecución.
...
#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>Proteger contra recursiva llamadas a las funciones en tiempo de ejecución correcciones
...
g
```
(ejecutar hasta que la aplicación de destino se inicia y entra en el depurador)
