---
description: Traduce la práctica de definición de interfaz de usuario en el formulario de marcado XAML declarativo muy bien en Windows Phone Silverlight para aplicaciones de plataforma Universal de Windows (UWP).
title: Migración de Windows Phone Silverlight XAML y la interfaz de usuario a UWP
...
Traduce la práctica de definición de interfaz de usuario en el formulario de marcado XAML declarativo muy bien en Windows Phone Silverlight para aplicaciones de plataforma Universal de Windows (UWP). Encontrarás que grandes secciones del marcado son compatibles, una vez hayas actualizado las referencias de clave de recurso del sistema, cambiado algunos nombres de tipos de elementos y cambiado "clr-namespace" por "using". Parte del código imperativo de la capa de presentación (modelos de vistas y código que manipula elementos de interfaz de usuario) también será sencillo de migrar.

## <a name="a-first-look-at-the-xaml-markup"></a>Una primera mirada al marcado XAML

El tema anterior, mostramos cómo copiar su XAML y archivos de código subyacente en las ventanas nuevo proyecto de Visual Studio 10. Uno de los primeros problemas que se resaltará en el diseñador de XAML de Visual Studio es que el elemento `PhoneApplicationPage` en la raíz del archivo XAML no es válido para un proyecto de la Plataforma universal de Windows (UWP). En el tema anterior, se guarda una copia de los archivos XAML que Visual Studio genera cuando crea el proyecto de Windows 10. Si abres esta versión de MainPage.xaml, verás que en la raíz está el tipo [**Page**](https://msdn.microsoft.com/library/windows/apps/br227503), que se encuentra en el espacio de nombres [**Windows.UI.Xaml.Controls**](https://msdn.microsoft.com/library/windows/apps/br227716). Por lo tanto, es posible cambiar todos los elementos `<phone:PhoneApplicationPage>` por `<Page>` (no te olvides de la sintaxis del elemento de propiedad) y eliminar la declaración `xmlns:phone` .

Un enfoque más general para buscar el tipo UWP que corresponde a un tipo de Windows Phone Silverlight, puede hacer referencia a [asignaciones Namespace y clase](wpsl-to-uwp-namespace-and-class-mappings.md).
...
En ocasiones, la corrección del código imperativo será tan mínima como cambiar el tipo de un parámetro. En otras ocasiones, necesitará usar las API de UWP en lugar de aplicaciones de API de .NET para Windows Runtime 8.x. Para identificar las API que se admiten, use el resto de esta guía de migración en combinación con [información general de aplicaciones de .NET para Windows Runtime 8.x](https://msdn.microsoft.com/library/windows/apps/xaml/br230302.aspx) y [referencia en tiempo de ejecución de Windows](https://msdn.microsoft.com/library/windows/apps/br211377).

Asimismo, si solo quieres llegar a la etapa donde se crea el proyecto, puedes establecer como comentario o código auxiliar cualquier código que no sea esencial. A continuación, recorrer en iteración, uno de los problemas a la vez y consulte los temas siguientes en esta sección (y el tema anterior: [Solución de problemas de](wpsl-to-uwp-troubleshooting.md)), hasta que los problemas de compilación y tiempo de ejecución son plancharla horizontal y el puerto está completando.

## <a name="adaptiveresponsive-ui"></a>Interfaz de usuario adaptativa y dinámica

Ya puede ejecutar la aplicación de Windows 10 en una potencialmente amplia gama de dispositivos: cada uno con su propio tamaño de pantalla y la resolución, querrá ir más allá de los pasos mínimos a portar su aplicación y desea adaptar la interfaz de usuario para buscar la mejor manera de esos dispositivos. Puedes usar la función adaptativa Visual State Manager para detectar dinámicamente el tamaño de ventana y cambiar el diseño en respuesta. Se muestra un ejemplo de cómo hacerlo en la sección [Interfaz de usuario adaptativa](wpsl-to-uwp-case-study-bookstore2.md) en el tema de caso práctico Bookstore2.
...
-   El [ **enlace** ](https://msdn.microsoft.com/library/windows/apps/br209820) clase no incluye las propiedades extendidas de formato disponibles en Windows Phone Silverlight. Sin embargo, aún puedes implementar [**IValueConverter**](https://msdn.microsoft.com/library/windows/apps/br209903) para proporcionar formato personalizado.
...
Las aplicaciones de Windows Phone Silverlight usan controles definidos en el **Microsoft.Phone.Controls** espacio de nombres y el **System.Windows.Controls** espacio de nombres. Las aplicaciones para UWP XAML usan controles definidos en el espacio de nombres [**Windows.UI.Xaml.Controls**](https://msdn.microsoft.com/library/windows/apps/br227716). La arquitectura y diseño de los controles XAML de UWP es prácticamente igual que los controles de Windows Phone Silverlight. No obstante, se han realizado algunos cambios para mejorar el conjunto de controles disponibles y para unificarlos con las aplicaciones de Windows. A continuación se muestran ejemplos específicos.
...
| LongListSelector con datos agrupados | Las funciones de Windows Phone Silverlight LongListSelector de dos maneras, que se pueden usar conjuntamente. En primer lugar, es capaz de mostrar datos agrupados por una clave como, por ejemplo, una lista de nombres agrupados por su letra inicial. En segundo lugar, es capaz de aplicar "zoom" entre dos vistas semánticas: la lista agrupada de elementos (por ejemplo, nombres) y una lista de únicamente las claves de grupo (por ejemplo, letras iniciales). Con el UWP, puedes mostrar datos agrupados con las [Directrices para controles de lista y cuadrícula](https://msdn.microsoft.com/library/windows/apps/mt186889). |
| LongListSelector con datos planos | Por motivos de rendimiento, en el caso de las listas muy largas, se recomienda LongListSelector en lugar de un cuadro de lista de Windows Phone Silverlight incluso para los datos sin formato, no agrupado. En una aplicación para UWP, se prefiere [GridView](https://msdn.microsoft.com/library/windows/apps/br242705) para listas de elementos largas independientemente de si los datos son susceptibles de agruparse. |
| Panorámica | El control Panorama de Windows Phone Silverlight se asigna a la [directrices para los controles de hub en las aplicaciones de Windows Runtime 8.x](https://msdn.microsoft.com/library/windows/apps/dn449149) e instrucciones para el control hub. <br/> Ten en cuenta que un control Panorama se ajusta automáticamente desde la última sección a la primera y su imagen de fondo se mueve en parallax en relación con las secciones. Las secciones [Hub](https://msdn.microsoft.com/library/windows/apps/dn251843) no se ajustan automáticamente y no se usa parallax. |
| Pivot | Es el equivalente UWP del control Pivot de Windows Phone Silverlight [Windows.UI.Xaml.Controls.Pivot](https://msdn.microsoft.com/library/windows/apps/dn608241). Está disponible para todas las familias de dispositivos. |
...
Windows Phone Silverlight posee un **RadialGradientBrush** tipo que no está presente en la UWP, aunque otros [ **pincel** ](/uwp/api/Windows.UI.Xaml.Media.Brush) son de tipos. En algunos casos, podrás obtener un efecto similar con un mapa de bits. Ten en cuenta que puedes [crear un pincel de degradado radial](https://msdn.microsoft.com/library/windows/desktop/dd756679) con Direct2D en una aplicación para UWP C++, XAML y [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) .

Windows Phone Silverlight posee el **System.Windows.UIElement.OpacityMask** propiedad, pero que la propiedad no es un miembro de la UWP [**UIElement** ](https://msdn.microsoft.com/library/windows/apps/br208911) tipo. En algunos casos, podrás obtener un efecto similar con un mapa de bits. Asimismo, puedes [crear una máscara de opacidad](https://msdn.microsoft.com/library/windows/desktop/ee329947) con Direct2D en una aplicación para UWP C++, XAML y [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274). No obstante, un caso de uso habitual para **OpacityMask** es usar un único mapa de bits que se adapte a temas claros y oscuros. Para los gráficos vectoriales, puedes usar pinceles de reconocimiento de tema del sistema (por ejemplo, los gráficos circulares que se muestran a continuación). No obstante, para crear un mapa de bits de reconocimiento de tema (por ejemplo, las marcas de verificación que se muestran a continuación) se requiere un enfoque diferente.

![un mapa de bits de reconocimiento de tema](images/wpsl-to-uwp-case-studies/wpsl-to-uwp-theme-aware-bitmap.png)

En una aplicación de Windows Phone Silverlight, la técnica consiste en utilizar una máscara alfa (en forma de un mapa de bits) como el **OpacityMask** para un **rectángulo** rellena con el pincel de primer plano:
...
Aquí, winrt\_check.png es una máscara alfa en forma de un mapa de bits, al igual que wpsl\_es check.png y muy bien podría ser el mismo archivo. Sin embargo, es posible que desea proporcionar varios tamaños diferentes de winrt\_check.png que se usará para diferentes factores de escala. Para obtener información sobre esto y para ver una explicación de los cambios en los valores de **Width** y **Height**, consulta [Píxeles de visualización o efectivos, distancia de visualización y factores de escala](#view-or-effective-pixels-viewing-distance-and-scale-factors) en este tema.

Un enfoque más general, que es adecuado si existen diferencias entre la forma de tema claro y oscuro de un mapa de bits, es usar dos recursos de imagen: uno con un primer plano oscuro (para el tema claro) y otro con un primer plano claro (para el tema oscuro). Para obtener más información acerca de cómo asignar un nombre de este conjunto de recursos de mapa de bits, consulte [adaptar los recursos de idioma, la escala y otros calificadores](../app-resources/tailor-resources-lang-scale-contrast.md). Una vez que se ha asignado correctamente un nombre a los conjuntos de archivos de imagen, puedes hacer referencia a ellos en el resumen, usando su nombre raíz, como este:

```xml
    <Image Source="Assets/winrt_check.png" Stretch="None"/>
```

En Windows Phone Silverlight, el **UIElement.Clip** propiedad puede ser cualquier forma que se puede expresar con una **geometría** y normalmente se serializa en el marcado XAML en el **StreamGeometry** minilenguaje. En UWP, el tipo de la propiedad [**Clip**](https://msdn.microsoft.com/library/windows/apps/br208919) es [**RectangleGeometry**](https://msdn.microsoft.com/library/windows/apps/br210259), de modo que solo se puede recortar una región rectangular. Permitir la definición de un rectángulo usando minilenguaje sería demasiado permisivo. Por lo tanto, para migrar una región de recorte en el marcado, reemplaza la sintaxis del atributo **Clip** y conviértela en sintaxis de elemento de propiedad similar al siguiente:
...
Al navegar a una página en una aplicación de Windows Phone Silverlight, utilice un esquema de direccionamiento de identificador uniforme de recursos (URI):
...
Definir la página de inicio para una aplicación de Windows Phone Silverlight en WMAppManifest.xml:
...
El lenguaje de diseño ha evolucionado para Windows 10 y en consecuencia han cambiado algunos estilos del sistema, y muchas claves de recursos del sistema han eliminado o cambiado de nombre. El editor de marcado XAML en Visual Studio resalta las referencias a las claves de recursos que no puede resolver. Por ejemplo, el editor de marcado XAML subrayará una referencia a la clave de estilo `PhoneTextNormalStyle` con una línea ondulada roja. Si no se corrige, la aplicación finalizará inmediatamente cuando intentes implementarla en el emulador o el dispositivo. Por tanto, es importante prestar atención a la corrección del marcado XAML. Encontrarás que Visual Studio es una herramienta excelente para detectar esos problemas.
...
Aplicaciones de Windows Phone Silverlight y Windows 10 difieren en el modo en que abstraen el tamaño y el diseño de los elementos de interfaz de usuario fuera el tamaño físico actual y la resolución de los dispositivos. Una aplicación de Windows Phone Silverlight usa píxeles de la vista para hacer esto. Con Windows 10, el concepto de píxeles de la vista se han perfeccionado en de los píxeles efectivos. Esta es una explicación de este término, lo que significa y el valor adicional que ofrece.
