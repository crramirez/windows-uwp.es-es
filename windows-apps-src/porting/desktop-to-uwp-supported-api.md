...
Las aplicaciones de escritorio empaquetadas pueden usar una amplia gama de API de Plataforma universal de Windows (UWP), incluso si no se realizó la transición a una aplicación para UWP. En este artículo se enumeran las clases disponibles que puede usar tu aplicación empaquetada.

La mayoría de las API de UWP funcionan bien en aplicaciones de escritorio empaquetadas. Sin embargo, algunas áreas de características aún no están totalmente probadas o no funcionan actualmente según lo previsto.
...
| Interfaz de usuario XAML | Windows 10, versiones 1809 y anteriores no admiten el hospedaje contenido UWP XAML en aplicaciones de escritorio empaquetadas. A partir de Windows 10, versión 1903, puede usar [XAML Islas](../xaml-platform/xaml-host-controls.md) para hospedar contenido de UWP XAML.  |
...
La siguiente es una lista completa de las API para UWP que se admiten en aplicaciones empaquetadas. Puede ver [API admitidas solo en las aplicaciones empaquetadas](#new) o [API admitidas en aplicaciones de escritorio y aplicaciones empaquetadas](#both).
