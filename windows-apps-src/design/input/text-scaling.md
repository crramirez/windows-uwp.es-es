...
label: Text scaling
template: detail.hbs
keywords: UWP, texto, ajuste de escala, accesibilidad, "accesibilidad", para mostrar, "Conversión de texto más grande", interacción del usuario, entrada
ms.date: 08/02/2018
ms.topic: article
...
Lectura de texto en una pantalla de equipo (de dispositivo móvil en un portátil al monitor de escritorio a la pantalla de Surface Hub giant) puede ser un reto para muchas personas. Por el contrario, algunos usuarios encuentran los tamaños de fuente que se usan en las aplicaciones y sitios web para que sea más grande que es necesario.

Para garantizar que el texto sea legible como sea posible para la gama más amplia de usuarios, Windows proporciona la capacidad para que los usuarios cambiar el tamaño de fuente relativa en el sistema operativo y en aplicaciones individuales. En lugar de usar una aplicación de lupa (que normalmente solo amplía todo el contenido dentro de un área de la pantalla y se presenta sus propios problemas de facilidad de uso), cambiar la resolución de pantalla o depender de escalado de PPP (que cambia el tamaño de todo el contenido en función de la pantalla y visualización típico distancia), un usuario puede acceder rápidamente a una opción de configuración para cambiar el tamaño de solo texto, que van desde el 100% (el tamaño predeterminado) hasta 225%.

## <a name="support"></a>Compatibilidad

Aplicaciones universales de Windows (ambos estándar y PWA), compatibilidad con texto de escala de manera predeterminada.

Si la aplicación para UWP incluye controles personalizados, superficies de texto personalizado, alturas de control codificados de forma rígida, los marcos anteriores o marcos de terceros 3, es probable que deba realizar algunas actualizaciones para garantizar una experiencia coherente y útil para los usuarios.  
...
Los usuarios pueden ajustar la escala de texto con el texto de la marca -> control deslizante más grande de la configuración de accesibilidad -> pantalla de visión o la pantalla.
...
¿Dijimos esto ya? Vale la pena de repetición: cuando sea posible, siempre usa los controles integrados que se proporcionan con los distintos marcos de aplicación de Windows para obtener la experiencia de usuario más amplia posible el mínimo esfuerzo.

Por ejemplo, todos los controles de texto UWP admiten la experiencia sin necesidad de personalización o plantillas de ajuste de escala de texto completo.
...
![Texto animado 100% a 225% de escala](images/coretext/text-scaling.gif)  
...
En este fragmento de código del ejemplo anterior, se usa el `Auto` y `*` los valores de ancho de un conjunto de columnas de cuadrícula y permite que la plataforma ajustan el diseño de la aplicación en función del tamaño de los elementos contenidos dentro de la cuadrícula.
...
Este ejemplo, usamos el `AcceptsReturn` y `TextWrapping` propiedades de cuadro de texto para garantizar que nuestro diseño es flexible como sea posible.
...
![Animar el texto escala del 100% a 225% con ajuste de texto](images/coretext/text-scaling-textwrap.gif)  
...
![Texto que se ajuste de escala de 100 a 225% con el recorte de texto](images/coretext/text-scaling-clipping-small.png)  
...
Aquí, agregamos una información sobre herramientas en un bloque de texto que no es compatible con ajuste de texto:
...
Al usar iconos basados en fuentes de énfasis o detalle en, deshabilitar el escalado en estos caracteres.
...
Controla el evento de sistema de [TextScaleFactorChanged](https://docs.microsoft.com/uwp/api/windows.ui.viewmanagement.uisettings.textscalefactorchanged) UISettings en tu entorno personalizados y controles. Este evento se genera cada vez que el usuario establece el factor de escala de texto de su sistema.

## <a name="summary"></a>Resumen

Este tema proporciona una visión general de soporte técnico de Windows de ajuste de escala de texto e incluye instrucciones de experiencia del usuario y de desarrollador sobre cómo personalizar la experiencia del usuario.
