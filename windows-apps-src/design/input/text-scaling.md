...
Lectura de texto en una pantalla de equipo (de dispositivos móviles para equipos portátiles y monitor de escritorio a la pantalla de gigante de un dispositivo Surface Hub) puede ser un reto para muchas personas. Por el contrario, algunos usuarios encuentran los tamaños de fuente que se usan en las aplicaciones y sitios web para que sea mayor de lo necesario.

Para garantizar que el texto sea legible como sea posible para la gama más amplia de usuarios, Windows proporciona la capacidad para que los usuarios cambiar el tamaño de fuente relativa en el sistema operativo y en aplicaciones individuales. En lugar de usar una aplicación de lupa (que normalmente solo se amplía todo el contenido dentro de un área de la pantalla y presenta sus propios problemas de facilidad de uso), cambiar resolución de pantalla o depender de escalado de PPP (que cambia el tamaño de todo el contenido en función de la pantalla y visualización típico distancia), un usuario puede acceder rápidamente a una opción de configuración para cambiar el tamaño de solo texto, que van desde el 100% (el tamaño predeterminado) hasta 225%.

## <a name="support"></a>Soporte

Aplicaciones universales de Windows (ambos estándar y PWA), compatible con texto de escala de manera predeterminada.
...
¿Dijimos esto ya? Es importante destacar: cuando sea posible, usa siempre los controles integrados proporcionados con los distintos marcos de aplicación de Windows para obtener la experiencia de usuario más amplia posible de la menor cantidad de esfuerzo.
...
Este es un fragmento de código desde una aplicación para UWP básica que incluye un par de controles de texto estándar:
...
En este fragmento de código del ejemplo anterior, se usa el `Auto` y `*` los valores de ancho de un conjunto de columnas de cuadrícula y dejar la plataforma ajustan el diseño de la aplicación en función del tamaño de los elementos contenidos dentro de la cuadrícula.
...
Este ejemplo, usamos la `AcceptsReturn` y `TextWrapping` propiedades de cuadro de texto para garantizar que nuestro diseño es flexible como sea posible.
...
![Texto de escala del 100% a 225% con ajuste de texto animado](images/coretext/text-scaling-textwrap.gif)  
...
Si el ajuste de texto no es el comportamiento preferido, la mayoría de los controles de texto permitir que el texto de los recortes o especificar puntos suspensivos para el comportamiento de recorte de texto. Recorte es preferible al puntos suspensivos como puntos suspensivos ocupan un espacio a sí mismos.

> [!NOTE]
> Si es necesario el texto de los recortes, recortar al final de la cadena, y no al principio.
...
![Texto de ajuste de escala del 100% a 225% con el recorte de texto](images/coretext/text-scaling-clipping-small.png)  
...
Aquí, agregamos una información sobre herramientas a un bloque de texto que no es compatible con ajuste de texto:
