...
Cuando se implementan patrones de control para un marco de código administrado, como una aplicación de UWP que usa C @ no__t-0 o Microsoft Visual Basic, se pueden usar .NET Framework interfaces para representar estos patrones en lugar de usar la representación de la interfaz COM. Por ejemplo, la interfaz de un patrón de Automatización de la interfaz de usuario para una implementación de un proveedor de Microsoft .NET del patrón **Invoke** es [**IInvokeProvider**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Automation.Provider.IInvokeProvider).
...
* [**GetParent**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.peers.automationpeer.getparent): Llama a [**FrameworkElement. Parent**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.parent) desde el propietario y busca el elemento del mismo nivel adecuado. Este no es un par de invalidación con un método "Core", por lo que no puedes cambiar este comportamiento.
