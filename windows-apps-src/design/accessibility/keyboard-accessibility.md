...
Puedes hacer que el orden de tabulación coincida con el orden visual ajustando el código XAML. O bien, puedes invalidar el orden de tabulación predeterminado configurando la propiedad [**TabIndex**](/uwp/api/windows.ui.xaml.controls.control.tabindex), como se muestra en el siguiente ejemplo de un diseño [**Grid**](/uwp/api/Windows.UI.Xaml.Controls.Grid) que usa la navegación mediante tabulación por columnas primero.
...
Asegúrate de que los elementos de la interfaz de usuario en los que pueda hacerse clic también puedan invocarse con el teclado. Para usar el teclado con un elemento de la interfaz de usuario, el elemento debe tener foco. Solo las clases que derivan de [**Control**](/uwp/api/Windows.UI.Xaml.Controls.Control) admiten navegación mediante tabulación y foco.
...
En los casos en los que un elemento que quieres usar en la interfaz de usuario no pueda tener foco, podrías crear tu propio control personalizado. Debes establecer la propiedad [**IsTabStop**](/uwp/api/windows.ui.xaml.controls.control.istabstop) en **true** para habilitar el foco y debes proporcionar una indicación visual del estado enfocado creando un estado visual que decore la interfaz de usuario con un indicador de foco. Sin embargo, suele ser más fácil usar la composición de controles para que el soporte de los modelos de automatización de la interfaz de usuario de Microsoft y automatización del mismo nivel, foco y detenciones de tabulación sea administrado por el control dentro del cual elijas que se componga tu contenido.

Por ejemplo, en lugar de administrar un evento de presión de puntero en una clase [**Image**](/uwp/api/Windows.UI.Xaml.Controls.Image), podrías encapsularlo en una clase [**Button**](/uwp/api/Windows.UI.Xaml.Controls.Button) para obtener compatibilidad para foco, teclado y puntero.
...
Para obtener más información sobre la implementación de las teclas de método abreviado, consulta [Teclas de método abreviado](/windows/win32/uxguide/inter-keyboard) en las Directrices para la interacción de la experiencia de usuarios de Windows.
...
Por lo general, para implementar un control de teclas personalizado para controles personalizados se incluye una invalidación de los métodos [**OnKeyDown**](/uwp/api/windows.ui.xaml.controls.control.onkeydown) y [**OnKeyUp**](/uwp/api/windows.ui.xaml.controls.control.onkeyup) como parte de la lógica de la clase.
...
Este es un ejemplo de XAML que procede de la plantilla XAML predeterminada para un [**botón**](/uwp/api/Windows.UI.Xaml.Controls.Button).
...
De momento, esto es solo la composición. Para controlar la visibilidad del indicador de foco, tienes que definir los estados visuales que alternan la propiedad [**Visibility**](/uwp/api/windows.ui.xaml.uielement.visibility). Esto se hace mediante [VisualStateManager](/uwp/api/Windows.UI.Xaml.VisualStateManager) y la propiedad adjunta VisualStateManager. VisualStateGroups, tal y como se aplica al elemento raíz que define la composición.
...
* [Interacciones de teclado](../input/keyboard-interactions.md)
