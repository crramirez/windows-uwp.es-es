...
Además de definir sus propios colores en la aplicación, también puede limitarse nuestros colores systematized a regiones deseadas en toda la aplicación mediante la etiqueta **ColorSchemeResources** . Esta API permite que no solo colorear y grandes grupos de tema de controles a la vez, establece algunas propiedades, pero también proporciona muchos otro sistema ventajas que normalmente no obtienen con la definición de sus propios colores personalizados manualmente:
...
- Los colores que se establece en **ColorSchemeResources** se organiza en cascada hacia abajo en todos los controles similares que también usan ese color del sistema
...
- Dark
  * Se mostrarán los cambios de color en el [tema oscuro](https://docs.microsoft.com/windows/uwp/design/style/color#dark-theme)

Configuración de x: Key garantiza que los colores de forma adecuada cambian el tema del sistema o la aplicación, si desea una apariencia personalizada diferente cuando en el tema.
...
Por ejemplo, si se definen dos colores del sistema - **SystemBaseLowColor** y **SystemBaseMediumLowColor** dentro de una cuadrícula y, a continuación, colocan los dos botones en la página: uno dentro de esa cuadrícula y fuera de una:
...
![colores del sistema con ámbito en botón](images/color/scopedcolors_cyan_button.png)

Sin embargo, dado que todos los colores de nuestro sistema demasiado en cascada hacia abajo en otros controles, establecer **SystemBaseLowColor** y **SystemBaseMediumLowColor** afectarán a más que simplemente botones. En este caso, los controles como **ToggleButton**, **RadioButton** y **control deslizante** también se verán afectados estos cambios de color del sistema, estos controles se deben colocar por encima de ámbito de la cuadrícula de ejemplo.
Si quieres definir el ámbito de un sistema color cambiar *a un único controles, solo* puedes hacerlo mediante la definición **ColorSchemeResources** dentro de los recursos de ese control:
...
Anidamiento de los colores del sistema también es posible y se realiza mediante la colocación **ColorSchemeResources** en los recursos de los elementos anidados dentro del marcado de tu diseño de la aplicación:
...
Ahora, todos los recursos de temas y colores personalizados pueden colocarse en un diccionario de recursos de **MyCustomTheme** único y ámbito cuando sea necesario sin tener que preocuparse de desorden adicional en el marcado de diseño.
