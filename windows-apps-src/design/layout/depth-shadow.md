...
ms.locfileid: "4360774"
---
# <a name="z-depth-and-shadow"></a>Profundidad Z y sombra

![profundidad true](images/elevation-shadow/depth.svg)

El sistema de profundidad Fluent usa los conceptos de físicos como 3D posicionamiento, delgada, y sombras convertirse la interfaz de usuario de forma digital pueden considerarse en un entorno físico más en capas. Profundidad Z o relativa a la profundidad y sombras son dos formas de incorporar profundidad a tu aplicación para UWP.

## <a name="what-is-z-depth"></a>¿Qué profundidad z?

Profundidad Z es la distancia entre dos superficies a lo largo del eje z, y muestra cómo cerrar es un objeto en el Visor.

![profundidad z](images/elevation-shadow/elevation.svg)

### <a name="why-use-z-depth"></a>¿Por qué usar profundidad z?

En el mundo físico, se suele centrarse en los objetos que están más próximos a nosotros. Podemos aplicar este impulso espacial a IU digital. Por ejemplo, si se coloca un elemento más cerca al usuario, el usuario instintivamente se centrará en el elemento. Mover la interfaz de usuario los elementos más cerca en el eje z, puedes establecer una jerarquía visual entre objetos, lo que ayuda a los usuarios a completar las tareas de forma natural y eficaz en la aplicación. 

![profundidad z en el menú de contenido](images/elevation-shadow/whyelevation.svg)

Además proporcionar una jerarquía visual significativa, profundidad z también permite crear experiencias que fluyan sin problemas desde 2D a entornos 3D, escalado de la aplicación en todos los dispositivos y factores de forma. 
...
En función de cómo nos perciben profundidad en el mundo físico, hay varias técnicas que pueden usarse para mostrar la proximidad en la interfaz de usuario digital.

- **Escala** Los objetos más aparecen más pequeños que objetos más cerca del mismo tamaño. Se trata de método es difícil demostrar eficazmente en el espacio 2D, por lo que generalmente no se recomienda. Sin embargo, puedes usar escala y [sombras](#what-is-shadow) para crear una simulación eficaz de objetos que se mueven más cerca del usuario en 2D.
...
- **Movimiento** Velocidad relativa puede usarse para demostrar la proximidad: objetos más cerca se muevan más rápidamente que los objetos distantes en segundo plano. Para obtener información sobre cómo implementar este efecto, consulta el [efecto Parallax](../motion/parallax.md).
    ...
    ![proximidad con un movimiento](images/elevation-shadow/elevation-motion.svg)

### <a name="recommendations-for-z-depth"></a>Recomendaciones para la profundidad z
...
## <a name="what-is-shadow"></a>¿Qué es sombra?

![sombra](images/elevation-shadow/shadow.svg)

Sombra es una forma para percibir elevación. Cuando hay luz por encima de un objeto con privilegios elevados, existe una sombra en la superficie a continuación. Cuanto mayor sea el objeto, el más grande y más suave se convierte en la sombra. Ten en cuenta que los objetos con privilegios elevados no es necesario tener sombras, pero sombras indicar la elevación.

En las aplicaciones para UWP, sombras deben solapen, no estéticos. Si las sombras reducen el foco y la productividad, a continuación, limitar el uso de sombra.
...
- Se adapta a los cambios de iluminación, tema del usuario, el entorno de aplicación y shell.
...
Estos son ejemplos de ThemeShadow en diferentes elevaciones con los temas claros y oscuros:
...
ThemeShadow automáticamente proyecta sombras cuando se aplican a cualquier elemento XAML en un [elemento emergente](/uwp/api/windows.ui.xaml.controls.primitives.popup). Transmitirá sombras sobre el contenido de la aplicación en segundo plano detrás de él y los elementos emergentes otros abiertos por debajo de él.

Para usar ThemeShadow con elementos emergentes, usa el `Shadow` propiedad para aplicar un ThemeShadow a un elemento XAML. A continuación, elevar el elemento de otros elementos detrás de ella, por ejemplo, usa el componente z de la `Translation` propiedad.
...
En este ejemplo se muestra un rectángulo en un elemento emergente proyecta su sombra sobre el contenido de la aplicación en segundo plano y los otros elementos emergentes detrás de ella:
...
2. Si varios elementos de receptor están en la misma elevación, a continuación, intenta combinarlas, como destino un elemento primario único en su lugar.
...
DropShadow no es automáticamente con capacidad de respuesta en su entorno y no usa fuentes de luz. Por ejemplo las implementaciones, consulta la [Clase de DropShadow](https://docs.microsoft.com/uwp/api/windows.ui.composition.dropshadow).
...
- Si tienen más avanzadas escenarios para sombras personalizadas, a continuación, usar DropShadow, lo que permite una mayor personalización.
