...
Un botón ofrece al usuario una forma de desencadenar una acción inmediata. Algunos de los botones se especializan para tareas específicas, como la navegación, acciones que se repiten o presentar los menús.

![Ejemplo de botones](images/controls/button.png)

El marco XAML proporciona un control button estándar, así como varios controles con el botón especializados.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Versión preliminar) Un botón de alternancia con dos caras. Activa o desactiva uno de los lados encendido/apagado y el otro lado, abre un menú.
...
No uses un botón cuando la acción sea navegar a otra página; usa un [elemento HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) en su lugar. Para obtener más información, consulta [Hipervínculos](hyperlinks.md).
...
Usa un **botón de división** si quieres que el usuario pueda iniciar una acción inmediata o elegir entre las opciones adicionales de forma independiente.
...
> **Vista previa**: DropDownButton requiere la [última compilación de Windows 10 Insider Preview y SDK](https://insider.windows.com/for-developers/) o [Biblioteca de la interfaz de usuario de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).
...
En este ejemplo se muestra cómo crear un botón de lista desplegable con un control flotante que contiene comandos para la alineación de párrafo en un RichEditBox. (Para obtener más información y código, consulta [cuadro de texto enriquecido](rich-edit-box.md)).
...
> **Vista previa**: botón de división requiere la [última compilación de Windows 10 Insider Preview y SDK](https://insider.windows.com/for-developers/) o [Biblioteca de la interfaz de usuario de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un [botón de división](/uwp/api/windows.ui.xaml.controls.splitbutton) tiene dos partes que puedan invocarse por separado. Una parte se comporta como un botón estándar e invoca una acción inmediata. La otra parte, invoca un control flotante que contiene las opciones adicionales que el usuario puede elegir.
...
El comportamiento típico de un botón en dos paneles es:

- Cuando el usuario hace clic en la parte de botón, controlar el evento Click para invocar la opción que actualmente está seleccionada en la lista desplegable.
...
En este ejemplo se muestra cómo crear un botón en dos paneles que se usa para cambiar el color de primer plano del texto seleccionado en un RichEditBox. (Para obtener más información y código, consulta [cuadro de texto enriquecido](rich-edit-box.md)).
...
> **Vista previa**: ToggleSplitButton requiere la [última compilación de Windows 10 Insider Preview y SDK](https://insider.windows.com/for-developers/) o [Biblioteca de la interfaz de usuario de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un [ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) tiene dos partes que puedan invocarse por separado. Una parte se comporta como un botón de alternancia que puede estar activada o desactivada. La otra parte, invoca un control flotante que contiene las opciones adicionales que el usuario puede elegir.

Por lo general, se usa un botón de división de alternancia para habilitar o deshabilitar una característica cuando la función tiene varias opciones que el usuario puede elegir. Por ejemplo, en un editor de documentos, se podría usar para activar las listas o desactivar, mientras se usa la lista desplegable para elegir el estilo de la lista.
...
- La propiedad [ToggleSplitButton.IsChecked](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischecked) es simplemente un **bool**, no un **bool aceptan valores NULL**.
- ToggleSplitButton tiene solo el evento de [IsCheckedChanged](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischeckedchanged) ; no tiene eventos **Checked** y **Unchecked** separados.

### <a name="example---toggle-split-button"></a>Por ejemplo, botón de división de alternancia

El siguiente ejemplo muestra cómo un botón de alternancia botón en dos paneles puede usarse para activar o desactivar el formato de lista y cambiar el estilo de la lista, en un RichEditBox. (Para obtener más información y código, consulta [cuadro de texto enriquecido](rich-edit-box.md)).

![Un botón de división de alternancia para seleccionar los estilos de lista](images/toggle-split-button-open.png)
