---
Description: Personaliza la vista integrada de escritura a mano para entrada de lápiz en texto que sea compatible con controles de texto de UWP tales como TextBox y RichEditBox (y controles, como AutoSuggestBox que proporcionan una experiencia similar de entrada de texto).
...
Personaliza la vista integrada de escritura a mano para entrada de lápiz en texto que sea compatible con controles de texto de UWP tales como [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) y [RichEditBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.richeditbox) y controles derivados de estos tales como [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox).
...
En este ejemplo, se escucha el evento [TextChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textbox.textchanged) de un [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) y se aplica la fuente seleccionada por el usuario si el cambio de texto se originó en el HandwritingView; si no, se aplica una fuente predeterminada.
...
Para acceder al control [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) de un control compuesto, usa la [VisualTreeHelper](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.visualtreehelper) API.

El fragmento de código XAML siguiente muestra un control [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox).
...
En el correspondiente código subyacente, se muestra cómo deshabilitar [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) en el control compuesto [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox).
...
En algunos casos, puede que debas asegurarte de que el [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) abarque elementos de la interfaz de usuario que, de lo contrario, tal vez no incluiría.
...
Como el StackPanel es ahora más grande que el TextBox, es posible que el [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) no obstruya todo el control compuesto.

![TextBox con dictado](images/handwritingview/textbox-with-dictation-handwritingview.png)

Para solucionar este problema, establece la propiedad PlacementTarget del [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) en el elemento de la interfaz de usuario con el que se debe alinear.
...
También puedes establecer el tamaño del [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview), lo que puede resultar útil cuando tengas que asegurarte de que la vista no obstruye una interfaz de usuario importante.
...
Para ello, enlazamos la propiedad MaxWidth del [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) al ancho del elemento de interfaz de usuario que debe obstruir.
...
El siguiente ejemplo muestra cómo escuchar los eventos [Opened](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.opened), [Closed](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.closed
) y [SizeChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.sizechanged) del [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) para establecer la posición de un [Popup](https://docs.microsoft.com/uwp/api/windows.ui.popups).
...
Como con todos los controles del marco de trabajo XAML, puedes personalizar la estructura visual y el comportamiento visual de un [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) para tus requisitos específicos.

Para ver un ejemplo completo de creación de una plantilla personalizada, consulta los procedimientos de [Crear controles de transporte personalizados](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/custom-transport-controls) o el [ejemplo de control de edición personalizado](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/CustomEditControl).
