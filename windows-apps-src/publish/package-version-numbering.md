...
> 
            **Nota**  En este tema se hace referencia a los "paquetes" pero, a menos que se indique lo contrario, las mismas reglas se aplican a los números de versión de los archivos .appx y .appxbundle.

## Números de versión para paquetes de Windows 10
...
> 
            **Nota**  La última (cuarta) sección del número de versión está reservada para el uso de la Tienda y debe dejarse en 0.
...
Puedes incluso proporcionar varios paquetes de Windows 10 con el mismo número de versión. Sin embargo, los paquetes con el mismo número de versión no pueden tener también la misma arquitectura, ya que la identidad completa que la Tienda usa para cada paquete debe ser exclusiva. Para obtener más información, consulta [**Identity**](https://msdn.microsoft.com/library/windows/apps/br211441).

Si proporcionas varios paquetes de Windows 10 que usen el mismo número de versión, se usará la arquitectura (en el orden x64, x86, ARM, independiente) para decidir cuál tiene un intervalo superior al determinar qué paquete se proporciona al dispositivo determinado. Al clasificar lotes de aplicaciones que usan el mismo número de versión, se detiene en cuenta la arquitectura de mayor rango. Un lote de aplicaciones con un paquete x64 tiene un rango superior a otro que solo contenga un paquete x86.

Esto ofrece mucha flexibilidad al evolucionar la aplicación a lo largo del tiempo. Puedes cargar y enviar paquetes nuevos con números de versión inferiores para agregar compatibilidad con dispositivos económicos no admitidos anteriormente, o agregar paquetes con números de versión más altos y dependencias más estrictas para aprovechar características del hardware o del sistema operativo. También puedes agregar paquetes con números de versión más altos a modo de actualización para algunos o para todos tus clientes.
...
### Ejemplo: pasar a un único paquete a lo largo de varios envíos
...
> 
            **Nota**  En todos los casos, los dispositivos de los clientes recibirán el paquete con el mayor número de versión que admitan. Por ejemplo, en el tercer envío antes descrito, todos los dispositivos de escritorio obtendrán la versión v1.1.10.0, aunque tengan la versión 10.0.10250.0 o superior del sistema operativo y, por lo tanto, también pudieran aceptar la versión v1.1.5.0. Dado que 1.1.10.0 es el número de versión más alto disponible para ellos, ese es el paquete que obtienen.

### Usar el número de versión para revertir a un paquete distribuido anteriormente para las nuevas adquisiciones

Si guardas copias de los archivos de paquete anteriores de Windows 10, podrás revertir el paquete de la aplicación en la Tienda a uno de ellos si descubres problemas con alguna versión. Esta es una manera temporal de limitar la molestia para tus clientes mientras corriges el problema.

Para ello, crea un nuevo envío. Quita el paquete problemático y carga el paquete anterior que quieras proporcionar en la Tienda. Los clientes que ya hayan recibido el paquete al que estás revirtiendo seguirán teniendo el paquete problemático (ya que el antiguo tiene un número de versión anterior). Sin embargo, así se evita que nadie pueda comprar el paquete problemático y mantiene la aplicación disponible en la Tienda.
...
## Números de versión para Windows 8.1 (y versiones anteriores) y paquetes de Windows Phone 8.1
...
> 
            **Nota**  Si también tienes paquetes de Windows 10, el número de versión de estos debe ser mayor que el de los paquetes de Windows 8, Windows 8.1 o Windows Phone 8.1 que quieras publicar o ya hayas publicado. Para obtener más información, consulta [Agregar paquetes para Windows 10 a una aplicación publicada anteriormente](guidance-for-app-package-management.md#adding-packages-for-windows-10-to-a-previously-published-app).
...
| x86, v1.0.0.0 <br> x64, v1.0.0.0            | x64, v1.0.0.1               | v1.0.0.0 para clientes con x86 <br> v1.0.0.1 para clientes con x64                 | Nada para clientes que ejecutan la aplicación en un equipo x86. <br> v1.0.0.0 se actualizará a v1.0.0.1 para clientes que ejecuten la aplicación en un equipo x64. <br> 
            **Nota**  Si la versión x86 de la aplicación se ejecuta en un equipo x64, la aplicación no se actualizará a la versión x64 a menos que el cliente la desinstale y la vuelva a instalar. |
