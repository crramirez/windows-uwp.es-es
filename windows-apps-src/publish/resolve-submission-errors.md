...
-   **Uno o varios ensamblados del paquete están ofuscados incorrectamente:** Use una herramienta diferente para realizar la ofuscación o quite la ofuscación. El proceso de compilación optimiza los ensamblados ofuscados pero, en ocasiones, algunos ensamblados se ofuscan con una herramienta que modifica el MSIL de una forma no compatible que provoca un error.
-   **El tamaño de uno o más métodos de la aplicación supera los 256 KB de IL:** Refactorice el método infractor en funciones más pequeñas. El tamaño de MSIL para métodos de un ensamblado se puede determinar mediante la herramienta ILDASM.
-   **Error de validación de la firma de nombre seguro para uno o varios ensamblados:** Este error suele producirse cuando se realiza la firma de nombre seguro con una clave diferente de la esperada en los metadatos del ensamblado. Firma con la clave correcta o quita la firma de nombre seguro.
-   **El paquete contiene ensamblados de modo mixto (con código administrado y nativo):** Los ensamblados de modo mixto no se admiten en Windows Phone. Quita los ensamblados de modo mixto del paquete y vuelve a enviar la aplicación.
-   **Un ensamblado Windows Phone 8,1 XAP o appx/appxbundle no es válido:** Asegúrese de que el archivo. winmd tiene al menos un punto de entrada público. Si es necesario, puedes usar cualquier aplicación descompilador para revisar el código y comprobar si hay puntos de entrada públicos.
...
Si ve un error que indica **que el nombre encontrado en el paquete no es ninguno de los nombres de aplicación reservados. Reserve el nombre de la aplicación o actualice el paquete con el nombre de aplicación correcto para este idioma**; puede deberse a que ha escrito un nombre incorrecto en el paquete. Este error también puede producirse si usa un nombre de aplicación que no ha reservado en el centro de Partners. Normalmente se puede resolver el error siguiendo estos pasos:
