...
| Área                   | Descripción                                                                                  |
|------------------------|----------------------------------------------------------------------------------------------|
| Total pagado este año   | El total combinado abonado a su año, en dólares estadounidenses, para todos sus programas.       |
...
- El filtro puede dar lugar a un error No hay datos disponibles. Probablemente significa que ha dejado el período de tiempo predeterminado seleccionado en tres meses y, a continuación, ha seleccionado un ID. de pago de una ganancia que está fuera de ese período. Amplíe el período de tiempo y vuelva a intentarlo.
...
| partnerCountryCode             | Ubicación o país del asociado que gana.                                                                                                  | All                                                            |
...
| quantityType                   | Indica el tipo de cantidad, por ejemplo, cantidad facturada, MAU                                                                             | All                                                            |
| earningType                    | Indica si es honorarios, devolución, cooperativo, venta, etc.                                                                                          | All                                                            |
...
| transactionType                | Tipo de transacción (por ejemplo, compra, reembolso, revocación, contracargo, etc.).                                                               | Tienda y Azure Marketplace                                    |
| localProviderSeller            | Proveedor o vendedor local del registro.                                                                                                          | Solo Tienda                                                     |
| taxRemitted                    | Importe de impuestos remitidos (ventas, uso o impuestos de IVA/GST).                                                                                   | Tienda y Azure Marketplace                                    |
| taxRemitModel                  | Entidad responsable de la emisión de impuestos (ventas, uso o impuestos de IVA/GST).                                                                    | Solo Tienda                                                     |
| storeFee                       | La cantidad retenida por Microsoft como honorarios por hacer que la aplicación o el complemento estén disponibles en la tienda.                                           | Solo Tienda                                                     |
| transactionPaymentMethod       | Instrumento de pago del cliente usado para la transacción (por ejemplo, tarjeta, facturación del operador móvil, PayPal, etc.)                                | Tienda y Azure Marketplace                                    |
...
![Exportar extractos históricos](images/pc-export-statements.png)
...
|-------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Origen de ingresos          | El origen de los ingresos, en función de dónde se haya producido la transacción (por ejemplo, Microsoft Store, almacén de Windows Phone, tienda Windows 8, publicidad, etc.)                  |
| Identificador del pedido                | Identificador único del pedido. Este id. permite identificar las transacciones de compra con sus respectivas transacciones que no son de compra (como las devoluciones, la anulaciones, etc.). El identificador del pedido será el mismo en ambos casos. Además, en el caso de un cargo dividido en el que se hayan usado varios métodos de pago para una sola compra, te permitirá vincular las transacciones de compra. |
...
| Id. del producto principal       | Identificador único del producto principal. Ten en cuenta que, si no hay un producto principal para la transacción, el identificador del producto principal es igual al identificador del producto.                                |
...
| Tipo de transacción        | Tipo de transacción (por ejemplo, compra, reembolso, revocación, contracargo, etc.).                                                                                              |
| Método de pago          | Instrumento de pago del cliente usado para la transacción (por ejemplo, tarjeta, facturación del operador móvil, PayPal, etc.)                                                               |
...
| Impuestos remitidos            | Importe de impuestos remitidos (ventas, uso o impuestos de IVA/GST).                                                                                                                  |
| Recibos netos            | Importe de la transacción menos impuestos.                                                                                                                                   |
...
| Payment                 | Ganancias por la aplicación menos la retención de impuestos aplicable (el importe se muestra en la moneda de la transacción). (No se incluye en el archivo. csv **reservado** ).                               |
