...
ms.locfileid: "59244301"
---
# <a name="package-flights"></a>Paquetes piloto

Puede usar los vuelos de paquete para distribuir paquetes específicos a un grupo limitado de evaluadores. Los paquetes que ya ha publicado en el Store se usará para otros clientes, por lo que no se interrumpa su experiencia.

Con el paquete de vuelos, solo los paquetes son diferentes; lo detalles del anuncio de Store será el mismo para todos los clientes. Cualquier persona de su grupo de vuelo recibirán los paquetes que se incluyen en el vuelo de paquete, mientras que los clientes que no están en el grupo de vuelo continúan recibiendo sus paquetes regulares (no no finales).  Si más adelante decide que desea que los paquetes de un vuelo de paquete a disposición a todos sus clientes, puede usar fácilmente los mismos paquetes en un envío no finales no. Tenga en cuenta que los vuelos de paquete deben pasar el [proceso de certificación](the-app-certification-process.md), al igual que cualquier envío.

Al configurar los vuelos de paquete, puede especificar las personas que deben obtener paquetes específicos agregándolos a un **grupo de usuarios conocidos** (conocido también como grupo de vuelo). Cualquier persona de un grupo piloto que use un dispositivo con una versión de Windows 10 compatible con paquetes piloto (la compilación 10586 o posterior de Windows.Desktop, la compilación 10586.63 o posterior de Windows.Mobile, o Xbox One) obtendrá paquetes de los paquetes piloto que designes para ese grupo en concreto. (Los vuelos de paquete pueden incluir los paquetes destinados a cualquier versión del sistema operativo, incluidos Windows 8.1 / Windows Phone 8.1 o versiones anterior si la aplicación publicada previamente ya es compatible con ellos). Cualquier persona que no esté agregada a uno de tus grupos de piloto o que use un dispositivo no compatible con los paquetes piloto, obtendrá paquetes del envío de versión final.
...
> [!NOTE]
> Si tienes más de un paquete piloto, deberás asignar una clasificación a cada uno de ellos. Para obtener más información, consulte [Add y rango adicional del paquete vuelos](#add-and-rank-additional-package-flights) a continuación.
...
Después de guardar los detalles del paquete piloto, verás la página de información general. Haz clic en **Paquetes** para especificar los paquetes que te gustaría incluir en el paquete piloto. Puede incluir los paquetes destinados a cualquier versión del sistema operativo que admita la aplicación.

Tienes la opción de seleccionar paquetes que estaban asociados con un envío publicado anterior (un envío de versión final o uno de tus otros paquetes piloto, si tienes más de uno). Si necesita cargar nuevos paquetes que se utilizará para este vuelo de paquete, puede cargarlos aquí (mediante el [mismo proceso que al cargar paquetes de aplicaciones a un envío no finales que no sea regular](upload-app-packages.md)). Haz clic en **Guardar** cuando termines de especificar los paquetes que se incluirán en este paquete piloto.
...
Recuerde también de que la información de inscripción de Store y disponibilidad de familia de dispositivos se basa en el envío no finales no. Los clientes de los grupos piloto solo podrán descargar la aplicación en una familia de dispositivos que sea compatible con el envío de versión final. Para obtener más información, consulta [Compatibilidad con familias de dispositivos](#device-family-support). 
...
> [!IMPORTANT]
> Durante la implementación gradualmente los paquetes en un vuelo de paquete, las personas que no están incluidas en el porcentaje que obtiene los nuevos paquetes de obtener los paquetes desde el envío de vuelo paquete anterior (a menos que haya un vuelo de rango superior a su disposición).
...
De manera predeterminada, tu paquete piloto se publicará y estará disponible para tu grupo de piloto tan pronto como se complete el proceso de certificación. Si desea cambiar la [fecha de publicación](set-app-pricing-and-availability.md#publish-date), puede hacerlo en el **Flight opciones** sección. Haz clic en **Guardar** para volver a la página de información general del paquete piloto. 
...
> [!NOTE]
> Las personas que tengan un paquete que solo esté disponible en un paquete piloto pueden asignar a la aplicación una clasificación de estrellas y dejar opiniones, aunque estas no se mostrarán a otros clientes. (Esto excluye 7.x heredado o 8.0 paquetes XAP; las valoraciones y opiniones de los miembros de los grupos de vuelos con esos paquetes a la izquierda será visibles para otros clientes). Puede ver las clasificaciones y comentarios de todos los clientes, los de los grupos de vuelo, incluidos en el **revisiones** y **comentarios** informes para la aplicación.
...
Tenga en cuenta que el envío no finales no tiene el rango siempre el más bajo (1). Es decir, aquellas personas que no estén en ninguno de tus grupos piloto solo podrán obtener paquetes del envío de versión final a través de la Tienda. Personas en un grupo de vuelo obtendrá siempre los paquetes desde el vuelo paquete mejor clasificado disponible para ellos (pero nunca el envío de no no finales, ya que tiene el rango más bajo). Esto proporciona flexibilidad para determinar cómo distribuir los paquetes a las personas que puedan ser miembros de más de uno de tus grupos piloto.
...
Para eliminar un paquete piloto que ya no quieras admitir, haz clic en su nombre desde la página Información general de la aplicación. En la página de información general del piloto, haz clic en **Modificar** y luego haz clic en el vínculo **Eliminar** para eliminar el paquete piloto. (Si tiene un envío no publicado del vuelo de paquete en curso, necesitará eliminar primero ese envío). Pueden tardar hasta 30 minutos para que esto se puede completar.
