...
- Agente de socket. Si su aplicación usa sockets para las conexiones a largo plazo, a continuación, cuando sale de primer plano, puede delegar la propiedad de un socket a un agente de socket del sistema. A continuación, el agente: la aplicación se activa cuando llega el tráfico en el socket; transfiere la propiedad a la aplicación; y la aplicación, a continuación, procesa el tráfico entrante.
...
**Las marcas y condiciones relacionados con la red**
...
Para obtener un ejemplo completo que demuestre el uso de la clase [**SocketActivityTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.SocketActivityTrigger) y del agente de sockets, consulta la muestra de [SocketActivityStreamSocket](https://go.microsoft.com/fwlink/p/?LinkId=620606). Se realiza la inicialización del socket Escenario1\_Connect.xaml.cs y la implementación de la tarea en segundo plano está en SocketActivityTask.cs.
...
Cuando uses [**MessageWebSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.MessageWebSocket) o [**StreamWebSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamWebSocket), debes registrar una instancia de clase [**WebSocketKeepAlive**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.WebSocketKeepAlive) como [**TaskEntryPoint**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundtaskbuilder.taskentrypoint) para KeepAliveTrigger para permitir que se anule la suspensión de la aplicación y que esta pueda enviar mensajes de conexión persistente al servidor (punto de conexión remoto) de forma periódica. Esto debe hacerse como parte del código de registro de la aplicación en segundo plano, así como en el manifiesto del paquete.
...
Debes tener en cuenta algunos aspectos especiales cuando uses [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger). Hay algunos patrones de uso y procedimientos recomendados específicos de transporte que debes seguir al usar [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con **ControlChannelTrigger**. Estos aspectos también afectan la manera de controlar la forma en que las solicitudes reciben paquetes en [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637).

**Nota**            La clase   [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) que usa SSL ya no se admite cuando se usa la característica de desencadenador de red y [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger).
 
Los siguientes patrones de uso y procedimientos recomendados deben cumplirse cuando uses [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger):

-   La aplicación puede necesitar establecer varias propiedades y encabezados en el objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) o [HttpClientHandler](https://go.microsoft.com/fwlink/p/?linkid=241638) en el espacio de nombres [System.Net.Http](https://go.microsoft.com/fwlink/p/?linkid=227894), antes de enviar la solicitud al URI específico.
-   Una aplicación puede necesitar una solicitud inicial para probar y configurar el transporte de manera apropiada antes de crear el transporte [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) que se va a usar con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger). Una vez que la aplicación determine que el transporte puede configurarse correctamente, se puede configurar un objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) como el objeto de transporte que se usará con el objeto **ControlChannelTrigger**. Este proceso está diseñado para evitar que algunos escenarios interrumpan la conexión establecida en el transporte. Si se usa SSL con un certificado, una aplicación puede solicitar que se muestre un cuadro de diálogo con la entrada de PIN o si hay varios certificados entre los cuales elegir. Es posible que se requiera la autenticación de proxy y servidor. Si la autenticación de proxy o servidor expira, la conexión puede cerrarse. Una forma en que la aplicación puede abordar estos problemas de expiración de la autenticación es establecer un temporizador. Cuando se requiere una redirección HTTP, no se garantiza que pueda establecerse una segunda conexión fiable. Una solicitud de prueba inicial asegurará que la aplicación pueda usar la dirección URL redirigida más reciente antes de usar el objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) como transporte con el objeto **ControlChannelTrigger**.

A diferencia de otros transportes de red, el objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) no se puede pasar directamente al método [**UsingTransport**](https://docs.microsoft.com/uwp/api/windows.networking.sockets.controlchanneltrigger.usingtransport) del objeto [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger). En cambio, debes crear específicamente un objeto [HttpRequestMessage](https://go.microsoft.com/fwlink/p/?linkid=259153) para usarlo con el objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) y **ControlChannelTrigger**. El objeto [HttpRequestMessage](https://go.microsoft.com/fwlink/p/?linkid=259153) se crea mediante el método [RtcRequestFactory.Create](https://go.microsoft.com/fwlink/p/?linkid=259154). Posteriormente, el objeto [HttpRequestMessage](https://go.microsoft.com/fwlink/p/?linkid=259153) creado se pasara al método **UsingTransport**.

En la siguiente muestra podrás ver cómo crear un objeto [HttpRequestMessage](https://go.microsoft.com/fwlink/p/?linkid=259153) para usarlo con el objeto [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) y la clase [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger).
...
Algunos aspectos especiales afectan a la manera en que se controla el envío de solicitudes HTTP en la clase [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637), a la hora de iniciar la recepción de una respuesta. En particular, cuando usas [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger), la aplicación debe usar un elemento "Task" para controlar los envíos en lugar de controlar el modelo **await**.

Si se usa [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637), no habrá sincronización entre el método [**IBackgroundTask.Run**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.ibackgroundtask.) de la tarea en segundo plano de [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger) y la devolución de llamada de finalización recibida. Por esta razón, la aplicación solo puede usar la técnica de bloqueo HttpResponseMessage en el método **Run** y esperar hasta que se reciba la respuesta completa.

Es notablemente diferente usar [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger), que usar los transportes [**StreamSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamSocket), [**MessageWebSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.MessageWebSocket) o [**StreamWebSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamWebSocket). La devolución de llamada recibida de [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) se entrega a la aplicación mediante un elemento "Task" desde el código [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637). Esto significa que la tarea de notificación de inserción **ControlChannelTrigger** se activará tan pronto como los datos o el error se despachen a la aplicación. En la siguiente muestra, el código almacenará el elemento responseTask que devolvió el método [HttpClient.SendAsync](https://go.microsoft.com/fwlink/p/?linkid=241637), en el almacenamiento global que la tarea de notificación de inserción tomará y procesará en línea.

En la siguiente muestra, podrás ver cómo controlar solicitudes de envío en [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) cuando se usa con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger).
...
En la siguiente muestra, se indica cómo leer las respuestas recibidas en [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) cuando se usa con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger).
...
Para obtener más información sobre cómo usar [HttpClient](https://go.microsoft.com/fwlink/p/?linkid=241637) con [**ControlChannelTrigger**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger), consulta [ControlChannelTrigger HttpClient sample (Muestra de ControlChannelTrigger HttpClient)](https://go.microsoft.com/fwlink/p/?linkid=258323).
