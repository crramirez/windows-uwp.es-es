...
Este tema se centra en cómo usar las clases de socket de Plataforma universal de Windows (UWP) que se encuentran en el espacio de nombres [**Windows.Networking.Sockets**](/uwp/api/Windows.Networking.Sockets?branch=live). Pero también puedes usar [2 de Windows Sockets (Winsock)](https://msdn.microsoft.com/library/windows/desktop/ms740673) en una aplicación para UWP.

> [!NOTE]
> Como consecuencia del [aislamiento de red](https://msdn.microsoft.com/library/windows/apps/hh770532.aspx), Windows deshabilita establecer una conexión de socket (Sockets o WinSock) entre dos aplicaciones para UWP que se ejecuten en el mismo equipo, ya sea mediante la dirección de bucle invertido local (127.0.0.0) o especificando explícitamente la dirección IP local. Para obtener más información acerca de los mecanismos por los que las aplicaciones para UWP pueden comunicarse entre sí, consulta [Comunicación entre aplicaciones](/windows/uwp/app-to-app/index?branch=live).

## <a name="build-a-basic-tcp-socket-client-and-server"></a>Crear un servidor y cliente de socket TCP básico
Un socket TCP (Protocolo de control de transmisión) proporciona transferencias de datos de red de bajo nivel en cualquier dirección para conexiones de larga duración. Los sockets TCP son la característica subyacente que usan la mayoría de los protocolos de red que se usan en Internet. Para demostrar las operaciones básicas de TCP, el código de ejemplo siguiente muestra un [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) y un [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) enviando y recibiendo datos a través de TCP para formar un servidor y un cliente de eco.
...
Construye un [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) y comienza a escuchar las conexiones TCP entrantes. El evento [**StreamSocketListener.ConnectionReceived**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener.ConnectionReceived) se genera cada vez que un cliente establece una conexión con el **StreamSocketListener**.

Construye también un [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live), establece una conexión con el servidor, envía una solicitud y recibe una respuesta.
...
Coloca el marcado XAML en `MainPage.xaml` y coloca la el código obligatorio dentro de tu clase `MainPage`.

Crear una nueva **Page** denominada `DatagramSocketPage`. Coloca el marcado XAML en `DatagramSocketPage.xaml` y coloca la el código obligatorio dentro de la clase `DatagramSocketPage`.
...
Esta sección muestra un par de técnicas de envío por lotes que se puede usar con un [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) o un [**DatagramSocket **](/uwp/api/Windows.Networking.Sockets.DatagramSocket?branch=live) conectado.
...
Este primer ejemplo de una técnica más eficiente solo es adecuado si estás usando C#. Cambia `OnNavigatedTo` para llamar a `BatchedSendsCSharpOnly` en lugar de `SendMultipleBuffersInefficiently`.
...
Un error en una operación [**DatagramSocket**](/uwp/api/Windows.Networking.Sockets.DatagramSocket?branch=live), [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) o [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) se devuelve como un valor **HRESULT**. Puedes pasar ese valor **HRESULT** al método [**SocketError.GetStatus**](/uwp/api/windows.networking.sockets.socketerror.getstatus?branch=live) para convertirlo en un valor de enumeración [**SocketErrorStatus**](/uwp/api/Windows.Networking.Sockets.SocketErrorStatus?branch=live).
...
El constructor [**HostName**](/uwp/api/Windows.Networking.HostName?branch=live) puede arrojar una excepción si la cadena pasada no es un nombre de host válido. Por ejemplo, contiene caracteres no permitidos, lo cual es probable si el usuario escribe el nombre de host en la aplicación. Construye un **HostName** dentro de un bloque try/catch. De esa forma, si se arroja una excepción, la aplicación puede notificar al usuario y solicitar un nuevo nombre de host.
...
* [Comunicación entre aplicaciones](/windows/uwp/app-to-app/index?branch=live)
