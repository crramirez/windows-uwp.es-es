...
| Función        | Descripción           |
| -------------------------------------------------------------- |--------------------------------|
| [Variación del tamaño del elemento de la interfaz de usuario](#ui-element-sizing)  | La Plataforma universal de Windows usa [píxeles efectivos y ajuste de escala](..\layout\design-and-ui-intro.md#effective-pixels-and-scaling) para escalar la interfaz de usuario según la distancia de visualización. Cuando se ejecuta una aplicación UWP en Xbox One, usa un factor de escala predeterminado adecuado para hacer que todos los elementos de la interfaz de usuario, tales como el texto y los controles comunes, sean fácilmente visibles desde el sofá al otro ladro de la sala. Entender la variación de tamaño y aplicarla a toda tu interfaz de usuario te ayudarán a optimizar tu aplicación para el entorno de 10 pies.  |
|  [Zona segura del televisor](#tv-safe-area) | No todos los televisores muestran contenido hacia el extremo de la pantalla por razones históricas así como tecnológicas. La UWP automáticamente evitará mostrar cualquier interfaz de usuario en áreas no seguras (áreas cercanas a los bordes de la pantalla) de forma predeterminada. Sin embargo, esto crea un efecto de "encajonamiento" en la que la interfaz de usuario queda encerrada por un marco. Para que tu aplicación sea verdaderamente envolvente en el televisor, es conveniente modificarla para que se extiende hasta los bordes de la pantalla en los televisores que lo admiten. |
| [Colores](#colors)  |  La UWP admite temas de colores y una aplicación que respeta el tema del sistema tendrá como valor predeterminado **oscuro** en Xbox One. Si tu aplicación tiene un tema de color específico, debes tener en cuenta que algunos colores no funcionan bien en los televisores y deben evitarse. Para obtener los mejores resultados posibles, considera la posibilidad de crear para tu aplicación una paleta de colores específicos para televisores. |
| [Controlador para juegos y control remoto](#gamepad-and-remote-control)      | Asegurarse de que la aplicación funcione bien tanto con el controlador para juegos como con el control remoto es el paso más importante en la optimización de experiencias de 10 pies. Admitir correctamente la navegación y la interacción con el teclado le ayuda a las entradas mediante controlador para juegos y control remoto a funcionar relativamente bien. Sin embargo, hay mejoras específicas para los controladores para juegos y los controles remotos que puedes hacer para optimizar la interacción con el usuario en un dispositivo en el que sus acciones son un poco limitadas. |
| [Modo de mouse](#mouse-mode)|En algunas interfaces de usuario, tales como mapas y superficies de dibujo, no es posible o práctico utilizar la navegación con foco XY. Para estas interfaces, UWP proporciona el **modo de mouse** para permitir al controlador para juegos y al control remoto navegar libremente, como el mouse de un equipo de escritorio.|
| [Foco visual](#focus-visual)  | El foco visual es el borde alrededor del elemento de interfaz de usuario que actualmente tiene el foco. Esto ayuda a orientar al usuario para que pueda navegar fácilmente por tu interfaz de usuario sin perderse. Mientras el foco visual funciona en múltiples dispositivos de Windows 10, debes asegurarte de que es fácil de ver en el televisor y que se ubica de manera predeterminada en un lugar con el que usuario interactuará con frecuencia. Si el foco no está claramente visible, el usuario podría perderse en tu interfaz de usuario y no tener una gran experiencia. Además, puedes aprovechar la [Superposición de cierre del elemento por cambio de foco](#light-dismiss-overlay) para llamar la atención a la interfaz de usuario con la que un usuario esté interactuando actualmente.  |
| [Interacción y navegación con foco XY](#xy-focus-navigation-and-interaction) | El UWP proporciona **navegación con foco XY** la cual permite al usuario navegar por la interfaz de usuario de tu aplicación. Sin embargo, esto limita al usuario a navegar hacia arriba, abajo, izquierda y derecha. En esta sección se describen recomendaciones para lidiar con esta y otras consideraciones. Puede que también tengas que usar la [participación del foco](#focus-engagement) (presionando el botón **Entrar/Seleccionar** para interactuar con un elemento de interfaz de usuario) a fin de poder usar algunos controles, tales como los controles deslizantes. Esto reduce el número de clics necesarios para llegar de un lado de la pantalla al otro. | 
| [Directrices sobre controles de la interfaz de usuario](#guidelines-for-ui-controls)  |  Existen ciertas mejoras que puedes realizar en tu aplicación que tienen sentido para todos los dispositivos de Windows 10, no solo para Xbox One u otras experiencias de 10 pies. En lugar de solo mejorar la experiencia de 10 pies, considera aplicar estos procedimientos recomendados en todos los dispositivos compatibles con la UWP, que son particularmente beneficiosos para los televisores.  |
...
Xbox One representa tu aplicación en 1080p (1920 x 1080 píxeles). Por lo tanto, al traer una aplicación desde otros dispositivos tales como un PC, 
asegúrate de que la interfaz de usuario se vea bien en 960 x 540 px al 100% de escala usando las [técnicas adaptables](https://msdn.microsoft.com/en-us/windows/uwp/layout/screen-sizes-and-breakpoints-for-responsive-design).
...
No importa si el usuario tiene un televisor con una mayor resolución&mdash;las aplicaciones de la UWP siempre se ajustarán a 1080p.
...
Puedes hacer esto con los márgenes negativos en el control [SplitView](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.splitview.aspx), 
que se utiliza normalmente como un bloque de creación del panel de navegación y los márgenes positivos en el contenido de `SplitView` para mantenerlo dentro del área segura del televisor.
...
> **Nota**&nbsp;&nbsp; Si usas este enfoque, ten en cuenta que el botón **Más** cambia el alto del `CommandBar` abierto si es necesario, a fin de mostrar las etiquetas de los `AppBarButton` debajo de los iconos. Te recomendamos que muevas las etiquetas a la *derecha* de sus iconos para evitar este cambio de tamaño.
...
Corregir los colores no seguros para el televisor individualmente ajustando sus valores RGB para que estén dentro del rango seguro del televisor se suele denominar **compresión de color**. Este método puede ser apropiado para una aplicación que no usa una paleta de colores enriquecida. Sin embargo, corregir los colores de que esta manera puede provocar que los colores entren en conflicto entre sí, lo cual no proporciona la mejor experiencia de 10 pies.

En su lugar, te recomendamos que uses **escalado** tras comprobar que tus colores son seguros para el televisor mediante un método tal como la compresión de color para optimizar tu paleta de colores para el televisor. 

<!--[v-lcap] This seems to contradict what you just said in the previous sentence-->

Esto implica ajustar la escala de todos los valores RGB de tus colores en un factor determinado para que queden dentro del rango seguro para el televisor. 
Ajustar la escala todos los colores de tu aplicación ayuda a evitar las colisiones de color y contribuye a una mejor experiencia de 10 pies.
...
> **Nota**&nbsp;&nbsp; El tema claro **SystemChromeMediumLowColor** y **SystemChromeMediumLowColor** tienen el mismo color a propósito y no como resultado de la compresión. 

<!--[v-lcap] Double check that you didn't mean to say something else in the sentence above-->

> **Nota**&nbsp;&nbsp; Los colores hexadecimales se especifican en **ARGB** (alfa rojo verde azul).
...
Sin embargo, algunos controles, tales como [WebView](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.webview.aspx) y 
[MapControl](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.maps.mapcontrol.aspx), 
requieren una interacción tipo mouse en la que los usuarios puedan mover el puntero libremente dentro de los límites del control. 
...
> **Nota**&nbsp;&nbsp; No se admiten solicitudes de un puntero cuando un control obtiene el foco.
...
> **Nota**&nbsp;&nbsp; El modo de mouse solo se admite en Xbox One con controlador para juegos/control remoto. En las otras familias de dispositivos y tipos de entrada se omite silenciosamente.
...
> **Nota**&nbsp;&nbsp; `RequiresPointer` es una nueva API y aún no está documentada. 
...
> **Nota**&nbsp;&nbsp; Si un control activa el modo de mouse cuando se utiliza, también debe solicitar la activación de `IsEngagementRequired="true"`; de lo contrario, nunca se activará el modo de mouse.

Cuando un control está en modo de mouse, sus controles anidados también estarán en modo de mouse. El modo solicitado de sus elementos secundarios se ignorará&mdash;no es posible que es un elemento primario esté en modo de mouse y un elemento secundario no.
...
> **Nota**&nbsp;&nbsp; El valor `WhenFocused` solo se admite en objetos [Página](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.page.aspx). Si se intenta establecer este valor en un control, se generará una excepción.
...
Siempre debe haber un foco visual visible en la pantalla para que el usuario pueda continuar desde donde estaba sin necesidad de buscar el foco. Del mismo modo, debe haber un elemento activable en la pantalla en todas momento&mdash;por ejemplo, no uses ventanas emergentes con solo texto y ningún elemento activable.
...
> **Nota**&nbsp;&nbsp; Configurar la participación del foco no afecta al teclado ni a los demás dispositivos de entrada.
...
> **Nota**&nbsp;&nbsp; `IsFocusEngagementEnabled` es una nueva API y aún no está documentada.
...
Esto permitirá al usuario saltear la `ListView` rápidamente simplemente presionando hacia abajo. Pero 
no podrán desplazarse por la lista o elegir un elemento de ella a menos que lo activen presionando el botón **A/Seleccionar** cuando tenga foco y, a continuación, presionando el botón **B/Atrás** para desactivarlo.

![ListView con participación requerida](images/designing-for-tv/focus-engagement-list-and-grid-controls-2.png)

#### ScrollViewer

El [ScrollViewer](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.scrollviewer.aspx) es ligeramente diferente de estos controles, 
y tiene sus propias peculiaridades a tener en cuenta. Si tienes un `ScrollViewer` con contenido activable, desplazándose al `ScrollViewer` te permitirá por defecto moverte por sus elementos activables. Como en `ListView`, debes desplazarte por cada elemento para navegar fuera de `ScrollViewer`. 

Si el `ScrollViewer` *no* tiene contenido activable&mdash;por ejemplo, si solo contiene texto&mdash; puedes establecer `IsFocusEngagementEnabled="True"` para que el usuario puede activar el `ScrollViewer` mediante el uso del botón **A/Seleccionar**. Después de haberlo activado, pueden desplazarse por el texto mediante el **pad-D/palanca izquierda**y, a continuación, presionar el botón **B/Atrás** para desactivarlo una vez que han terminado.

Otro enfoque sería establecer `IsTabStop="True"` en `ScrollViewer` para que el usuario no tenga que activar el control&mdash;sino simplemente 
colocar el foco en él y, a continuación, desplazarse con el **pad-D/palanca izquierda** cuando no haya ningún elemento activable dentro del `ScrollViewer`.
...
La navegación con las teclas de dirección se asigna al **pad-D** (así como a la **palanca izquierda** en el controlador para juegos), y la interacción con los elementos de la interfaz de usuario se asigna a la tecla **Entrar/Seleccionar** 
(Consulta [Controlador para juegos y control remoto](#gamepad-and-remote-control)). Para obtener directrices de diseño para teclado, consulta [Interacciones con el teclado](keyboard-interactions.md).
...
> **Nota**&nbsp;&nbsp;Esta aplicación imaginaria pretende ilustrar los problemas en la interfaz de usuario y sus posibles soluciones y no está pensada para mostrar la mejor experiencia de usuario para tu aplicación en particular.
...
La [ListView](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.listview.aspx) de las propiedades que se muestra en la siguiente imagen es una lista con desplazamiento muy larga. Si la [participación](#focus-engagement) *no* se requiere en la `ListView`, cuando el usuario navegue a la lista, el foco se colocará en el primer elemento en la lista. Para que el usuario llegue al botón **anterior** o **siguiente**, deberá pasar por todos los elementos de la lista. En casos como este en el que resulta difícil solicitarle al usuario que recorra toda la lista&mdash;es decir, cuando la lista no es lo suficientemente corta como para esta experiencia sea aceptable&mdash;sería conveniente considerar otras opciones.
...
Dado que la navegación con foco XY depende de navegar a un elemento activable de la interfaz de usuario por vez, 
un [ScrollViewer](https://msdn.microsoft.com/en-us/library/windows/apps/windows.ui.xaml.controls.scrollviewer.aspx) que no contiene elementos activables (solo con texto, como en este ejemplo) puede causar un escenario en donde el usuario no puede ver todo el contenido del `ScrollViewer`. 
...
La Plataforma universal de Windows proporciona muchas funcionalidades que mejoran la experiencia del usuario en todos los dispositivos, algunas de las cuales benefician especialmente la experiencia de 10 pies. La siguiente lista describe algunos ejemplos de estas mejoras que puedes realizar a la interfaz de usuario de tu aplicación.
