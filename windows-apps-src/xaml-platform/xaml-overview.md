...
Este es un fragmento de código que muestra una <xref:Windows.UI.Xaml.Controls.Page> raíz creada por una plantilla de la página inicial de una aplicación (que solo muestra la etiqueta de apertura y simplificada). Declara el espacio de nombres predeterminado y, también, el espacio de nombres **x** (que explicaremos a continuación).
...
Windows Runtime también admite la extensión de marcado [{x:Null}](x-null-markup-extension.md). Se usa para establecer valores [**Nullable**](/dotnet/api/system.nullable-1) en **null** en XAML. Por ejemplo, podría utilizar esto en una plantilla de control para una <xref:Windows.UI.Xaml.Controls.CheckBox>, que interpreta **null** como un estado de comprobación indeterminado (que desencadena el estado visual "indeterminado").
