...
[**Ruta de acceso** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.path) describe la propiedad que se está enlazando (la propiedad de origen). Path es un parámetro de posición, lo que significa que puedes usar el nombre del parámetro explícitamente (`{Binding Path=EmployeeID}`), o bien, puede especificarlo como el primer parámetro sin nombre (`{Binding EmployeeID}`).
...
|----------|-------------|
| [**Ruta de acceso**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.path) | Consulta la sección [Ruta de acceso de propiedades](#property-path) anteriormente. |
...
| [**FallbackValue**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.fallbackvalue) | Especifica un valor que se mostrará cuando no se pueda resolver el origen o la ruta de acceso. |
| [**Modo**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.mode) | Especifica el modo de enlace, como uno de estos valores: "Una vez", "OneWay" o "TwoWay". Estos se corresponden con los nombres de constante de la enumeración de [**BindingMode**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Data.BindingMode). El valor predeterminado es "OneWay". Ten en cuenta que esto difiere del valor predeterminado de **{x:Bind}** , que es "OneTime". | 
...
[**Convertidor**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converter), [ **ConverterLanguage** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterlanguage) y **ConverterLanguage** están relacionadas con el escenario de convertir un valor o escriba desde el origen de enlace en un tipo o un valor que sea compatible con la propiedad de destino de enlace. Para obtener más información y ejemplos, consulta la sección “Conversiones de datos” de [Enlaces de datos en profundidad](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-in-depth).
...
El segundo ejemplo establece cuatro diferentes [ **enlace** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Data.Binding) propiedades: [**ElementName**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.elementname), [ **ruta**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.path), [ **modo** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.mode) y [ **convertidor** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converter). **Path** en este caso se muestra explícitamente como una propiedad **Binding**. **Path** se evalúa en un origen de enlace de datos que es otro objeto del mismo árbol de objetos en tiempo de ejecución, un [**Slider**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Slider) denominado `sliderValueConverter`.
