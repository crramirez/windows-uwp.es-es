...
Las API que tienen el atributo [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute) se admiten en todas las aplicaciones de escritorio, tengan o no la identidad de paquete. Para determinar si se admite una API a la que quieres llamar desde la aplicación de escritorio, busca el artículo de referencia de la clase o el miembro que te interesa en la [documentación de referencia de los espacios de nombres de WinRT](/uwp/api/). A continuación, comprueba si la sección **Attributes** del artículo de referencia incluye el atributo [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute).
